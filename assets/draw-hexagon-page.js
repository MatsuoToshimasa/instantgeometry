(function () {
  ...
  function formatNumber(value) {
    if (window.InstantGeometryDrawSettings && typeof window.InstantGeometryDrawSettings.formatNumber === 'function') return window.InstantGeometryDrawSettings.formatNumber(value);
    const rounded = Math.round(value * 100) / 100; return Number.isInteger(rounded) ? String(rounded) : rounded.toFixed(2).replace(/0+$/, '').replace(/\.$/, '');
  }
  function appendUnit(text, square) {
    ...
    const unit = unitOptions[unitIndex]; return unit ? (text + unit + (square ? '²' : '')) : text;
  }
  ...
  rightDock.classList.toggle('is-collapsed', isRightDockCollapsed);
  updateRatioButton(); syncDrawSettingsState(); updateDockToggleButtons(); renderLabelToggleButtons(); render();
})();
