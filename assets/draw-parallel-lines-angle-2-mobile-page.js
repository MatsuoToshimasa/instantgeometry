(function () {
  ...
  const POINT_IDS = ['P', 'Q', 'R', 'S', 'A', 'B', 'M', 'N'];
  const SEGMENTS = {
    PQ: ['P', 'Q'],
    RS: ['R', 'S'],
    AM: ['A', 'M'],
  ...
  const rbnInput = document.getElementById('rbnInput');
  const bnmInput = document.getElementById('bnmInput');
  const nmaInput = document.getElementById('nmaInput');
  const maqInput = document.getElementById('maqInput');

  const state = {
    points: { P: 'P', Q: 'Q', R: 'R', S: 'S', A: 'A', B: 'B', M: 'M', N: 'N' },
    pointVisible: { P: false, Q: false, R: false, S: false, A: false, B: false, M: false, N: false },
    segmentInputs: { PQ: '', RS: '', AM: '', MN: '', NB: '' },
    segmentKinds: { PQ: 'plain', RS: 'plain', AM: 'plain', MN: 'plain', NB: 'plain' },
    segmentArcVisible: { PQ: true, RS: true, AM: true, MN: true, NB: true },
    angleInputs: { MAQ: ' ', NMA: ' ', BNM: ' ', RBN: ' ' },
  ...
  function parseAngleInput(input, label) {
    ...
    if (!/^[1-9][0-9]*$/.test(text)) throw new Error(label + ' は整数で入力してください。');
    const value = Number(text);
    if (!(value > 0 && value < 180)) throw new Error(label + ' は 0° より大きく 180° 未満で入力してください。');
    return value;
  }

  function pointFrom(point, degrees, length) {
  ...
  function pointOnRayAtY(start, travelDegrees, y) {
    const radians = travelDegrees * Math.PI / 180;
    const dy = Math.sin(radians);
    if (Math.abs(dy) < 1e-9) return null;
    const t = (y - start.y) / dy;
    if (!(t > 0)) return null;
    return {
      x: start.x + Math.cos(radians) * t,
      y: y
    };
  }

  function intersectRayWithY(start, degrees, y, constraints) {
    const candidates = [degrees, 360 - degrees]
      .filter(function (value, index, values) { return values.indexOf(value) === index; })
      .map(function (travelDegrees) { return pointOnRayAtY(start, travelDegrees, y); })
      .filter(Boolean);
    if (!candidates.length) throw new Error('指定した角では平行線に接続できません。');
    if (!constraints) return candidates[0];
    const matched = candidates.find(function (point) {
      return signedAngleDiff(angleValue(constraints.R, point, start), constraints.rbn) < 0.001
        && signedAngleDiff(angleValue(point, start, constraints.M), constraints.bnm) < 0.001;
    });
    if (!matched) throw new Error('指定した角では入力された ∠BNM と ∠RBN を同時に作れません。');
    return matched;
  }

  function computeGeometry() {
    const rbn = parseAngleInput(rbnInput, '∠RBN');
    const bnm = parseAngleInput(bnmInput, '∠BNM');
    const nma = parseAngleInput(nmaInput, '∠NMA');
    const maq = parseAngleInput(maqInput, '∠MAQ');
    const expectedRbn = maq + bnm - nma;
    if (Math.abs(expectedRbn - rbn) > 0.0001) {
      throw new Error('4つの角は ∠RBN = ∠MAQ + ∠BNM - ∠NMA となるようにしてください。');
    }
    ...
    const thetaAM = maq;
    const thetaMN = maq + 180 - nma;
    const thetaNB = rbn;
    const A = { x: -2.6, y: upperY };
    const M = pointFrom(A, thetaAM, 1.75);
    const N = pointFrom(M, thetaMN, 1.95);
    const lowerLeft = { x: leftX, y: lowerY };
    const B = intersectRayWithY(N, thetaNB, lowerY, { M: M, R: lowerLeft, bnm: bnm, rbn: rbn });
    const maxX = lineLength / 2 - 0.25;
    const minX = leftX + 0.25;
    if (B.x > maxX || B.x < minX) {
      throw new Error('指定した角では点Bが画面内に収まりません。');
    }
    const points = {
      P: { x: leftX, y: upperY },
    ...
    return {
      points: points,
      angles: {
        MAQ: angleValue(M, A, points.Q),
        NMA: angleValue(N, M, A),
        BNM: angleValue(B, N, M),
        RBN: angleValue(points.R, B, N)
      }
    };
  }
  ...
  function render() {
    try {
      ...
      setStatus('PQ と RS の間に M、N をもつ平行線と角②を描画しています。', false);
    } catch (error) {
      stage.innerHTML = '';
  ...
  async function saveAs(format) {
    ...
    if (format === 'png' || format === 'transparent') {
      ...
      link.download = format === 'transparent' ? 'parallel-lines-angle-2-transparent.png' : 'parallel-lines-angle-2.png';
      link.click();
      return;
    }
    ...
    pdf.save('parallel-lines-angle-2.pdf');
  }
