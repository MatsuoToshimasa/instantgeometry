(function () {
  ...
  const state = {
    points: { P: 'P', Q: 'Q', R: 'R', S: 'S', A: 'A', B: 'B', C: 'C' },
    pointVisible: { P: false, Q: false, R: false, S: false, A: false, B: false, C: false },
    segmentInputs: { PQ: '', RS: '', AC: '', BC: '' },
    segmentKinds: { PQ: 'plain', RS: 'plain', AC: 'plain', BC: 'plain' },
    segmentArcVisible: { PQ: true, RS: true, AC: true, BC: true },
    angleInputs: { CAP: ' ', CBR: ' ', CAB: ' ', CAQ: ' ', CBS: ' ' },
    angleKinds: { CAP: 'plain', CBR: 'plain', CAB: 'plain', CAQ: 'plain', CBS: 'plain' }
  };
  ...
  function rayDirection(degrees) {
    const radians = degrees * Math.PI / 180;
    return { x: Math.cos(radians), y: Math.sin(radians) };
  }

  function intersectRays(origin1, degrees1, origin2, degrees2) {
    const d1 = rayDirection(degrees1);
    const d2 = rayDirection(degrees2);
    const denominator = d1.x * d2.y - d1.y * d2.x;
    if (Math.abs(denominator) < 1e-9) throw new Error('指定した角では AC と BC が交わりません。');
    const diff = { x: origin2.x - origin1.x, y: origin2.y - origin1.y };
    const t = (diff.x * d2.y - diff.y * d2.x) / denominator;
    const u = (diff.x * d1.y - diff.y * d1.x) / denominator;
    if (!(t > 0 && u > 0)) throw new Error('指定した角では点 C が作れません。');
    return { x: origin1.x + d1.x * t, y: origin1.y + d1.y * t };
  }

  function computeGeometry() {
    const cap = parseAngleInput(capInput, '∠CAP');
    const cbr = parseAngleInput(cbrInput, '∠CBR');
    const lineLength = 10;
    const upperY = -1.65;
    const lowerY = 2.35;
    const leftX = -lineLength / 2;
    const rightX = lineLength / 2;
    const A = { x: -2.4, y: upperY };
    const B = { x: 1.35, y: lowerY };
    const C = intersectRays(A, 180 + cap, B, 180 + cbr);
    if (!(C.y < upperY)) throw new Error('指定した角では点 C が PQ の上にできません。');
    if (C.x < leftX - 0.5 || C.x > rightX + 0.5) throw new Error('指定した角では点 C が画面内に収まりません。');
    const points = {
      P: { x: leftX, y: upperY },
      Q: { x: rightX, y: upperY },
      R: { x: leftX, y: lowerY },
    ...
    return {
      points: points,
      angles: {
        CAP: angleValue(C, A, points.P),
        CBR: angleValue(C, B, points.R),
        CAB: angleValue(C, A, B),
        CAQ: angleValue(C, A, points.Q),
        CBS: angleValue(C, B, points.S)
      }
    };
  }
  ...
  function render() {
    try {
      ...
      setStatus('∠CAP と ∠CBR から、PQ、RS、AC、BC を描画しています。', false);
    } catch (error) {
      stage.innerHTML = '';
  ...
  async function saveAs(format) {
    ...
    if (format === 'png' || format === 'transparent') {
      ...
      link.download = format === 'transparent' ? 'parallel-lines-angle-3-transparent.png' : 'parallel-lines-angle-3.png';
      link.click();
      return;
    }
    ...
    pdf.save('parallel-lines-angle-3.pdf');
  }
  ...
  [capInput, cbrInput].forEach(function (input) {
