(function () {
  ...
  const lengthInputs = { BE: beInput, EF: efInput, FC: fcInput };
  ...
  const state = {
    ...
    segmentInputs: {
      AB: '', AD: '', CD: '',
      BE: 'ratio:r,1', EF: 'ratio:r,1', FC: 'ratio:r,1',
      BG: 'ratio:s,5', GH: 'ratio:s,3', HD: 'ratio:s,12'
    },
    segmentKinds: {},
    segmentArcVisible: {},
  ...
  function computeGeometry() {
    const be = parsePositiveNumber(beInput.value, 'BE');
    const ef = parsePositiveNumber(efInput.value, 'EF');
    const fc = parsePositiveNumber(fcInput.value, 'FC');

    const A = { x: -3.7, y: -2.05 };
    const B = { x: 3.6, y: -2.05 };
    const C = { x: 4.8, y: 2.05 };
    const D = { x: -2.5, y: 2.05 };
    const total = be + ef + fc;
    const E = mix(B, C, be / total);
    const F = mix(B, C, (be + ef) / total);
  ...
  function render() {
    try {
      ...
      setStatus('BE、EF、FC から、平行四辺形内の連比②を描画しました。', false);
    } catch (error) {
      setStatus(error.message || '入力を確認してください。', true);
    }
  }
  ...
  function openSegmentSheet(id) {
    ...
    if (geometry && LABEL_SEGMENTS[id]) {
      const pair = LABEL_SEGMENTS[id];
      const numberField = document.createElement('div');
      numberField.className = 'sheet-field';
      const numberLabel = document.createElement('label');
      numberLabel.textContent = '数値';
      const numberInput = document.createElement('input');
      numberInput.type = 'text';
      numberInput.value = formatNumber(segmentLength(geometry.points[pair[0]], geometry.points[pair[1]]));
      numberInput.setAttribute('readonly', 'readonly');
      numberField.appendChild(numberLabel);
      numberField.appendChild(numberInput);
      sheetBody.appendChild(numberField);
    }
  ...
  function openAreaSheet(id) {
    ...
    if (area) {
      ...
      const numberInput = document.createElement('input');
      numberInput.type = 'text';
      numberInput.value = formatNumber(area.value);
      numberInput.setAttribute('readonly', 'readonly');
      numberField.appendChild(numberLabel);
      numberField.appendChild(numberInput);
      sheetBody.appendChild(numberField);
  ...
  async function saveAs(format) {
    ...
    if (format === 'png' || format === 'transparent') {
      ...
      link.download = format === 'transparent' ? 'parallelogram-ratio-2-transparent.png' : 'parallelogram-ratio-2.png';
      link.click();
      return;
    }
    ...
    pdf.save('parallelogram-ratio-2.pdf');
  }
