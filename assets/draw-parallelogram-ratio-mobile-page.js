(function () {
  ...
  const POINT_IDS = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'];
  const DRAW_SEGMENTS = {
    AB: ['A', 'B'], BC: ['B', 'C'], CD: ['C', 'D'], DA: ['D', 'A'],
    BD: ['B', 'D'], AE: ['A', 'E'], AF: ['A', 'F']
  };
  const LABEL_SEGMENTS = {
    AB: ['A', 'B'], BC: ['B', 'C'], CD: ['C', 'D'], DA: ['D', 'A'],
    BD: ['B', 'D'], AE: ['A', 'E'], AF: ['A', 'F'],
    BE: ['B', 'E'], EC: ['E', 'C'], CF: ['C', 'F'], FD: ['F', 'D'],
    BG: ['B', 'G'], GH: ['G', 'H'], HD: ['H', 'D'],
    AG: ['A', 'G'], GE: ['G', 'E'], AH: ['A', 'H'], HF: ['H', 'F']
  };
  const AREA_REGIONS = {
    ABG: ['A', 'B', 'G'],
    AGH: ['A', 'G', 'H'],
  ...
  const beInput = document.getElementById('beInput');
  ...
  const ratioInputs = { BE: beInput, EC: ecInput, CF: cfInput, FD: fdInput };

  const state = {
    pointInputs: { A: 'A', B: 'B', C: 'C', D: 'D', E: 'E', F: 'F', G: 'G', H: 'H' },
    pointVisible: { A: true, B: true, C: true, D: true, E: true, F: true, G: true, H: true },
    segmentInputs: {
      AB: '', BC: '', CD: '', DA: '',
      BD: '', AE: '', AF: '',
      BE: 'ratio:r,1', EC: 'ratio:r,1', CF: 'ratio:s,1', FD: 'ratio:s,2',
      BG: 'ratio:t,5', GH: 'ratio:t,4', HD: 'ratio:t,6', AG: '', GE: '', AH: '', HF: ''
    },
    segmentKinds: {},
    segmentArcVisible: {},
    areaInputs: { ABG: '', AGH: '', AHD: '', EBG: '', FDH: '', CFHGE: '' },
    areaColors: {}
  };
  ...
  function computeGeometry() {
    const be = parseRatioLabelInput(beInput.value, 'BE');
    const ec = parseRatioLabelInput(ecInput.value, 'EC');
    const cf = parseRatioLabelInput(cfInput.value, 'CF');
    const fd = parseRatioLabelInput(fdInput.value, 'FD');
    state.segmentInputs.BE = RATIO_LABEL_PREFIX + be.source;
    state.segmentInputs.EC = RATIO_LABEL_PREFIX + ec.source;
    state.segmentInputs.CF = RATIO_LABEL_PREFIX + cf.source;
    state.segmentInputs.FD = RATIO_LABEL_PREFIX + fd.source;

    const A = { x: -3.7, y: -2.05 };
    const B = { x: 3.6, y: -2.05 };
    const C = { x: 4.8, y: 2.05 };
    const D = { x: -2.5, y: 2.05 };
    const E = mix(B, C, be.number / (be.number + ec.number));
    const F = mix(C, D, cf.number / (cf.number + fd.number));
    const G = intersectLines(A, E, B, D);
    const H = intersectLines(A, F, B, D);
    return { points: { A: A, B: B, C: C, D: D, E: E, F: F, G: G, H: H } };
  }
  ...
  function areaRegions() {
    return Object.keys(AREA_REGIONS).map(function (id) {
      ...
      return {
        id: id,
        name: (AREA_REGIONS[id].length === 3 ? '△' : '五角形') + id,
        points: areaPoints,
  ...
  function render() {
    try {
      ...
      Object.keys(LABEL_SEGMENTS).forEach(function (id) {
        ...
        drawSegment(id, points[pair[0]], points[pair[1]], Object.prototype.hasOwnProperty.call(DRAW_SEGMENTS, id));
      ...
      POINT_IDS.forEach(function (id) {
        ...
        stage.appendChild(createSvg('circle', {
          cx: formatNumber(p.x), cy: formatNumber(p.y), r: id === 'G' || id === 'H' ? 6.5 : 8,
          fill: 'rgb(31,36,48)', 'data-kind': 'point', 'data-id': id
        }));
        if (!state.pointVisible[id]) return;
        const offsets = {
          A: [-32, -34], B: [32, -34], C: [34, 30], D: [-34, 30],
          E: [34, 0], F: [0, 34], G: [-26, 24], H: [28, 24]
        };
        drawText('point', id, state.pointInputs[id], p.x + offsets[id][0], p.y + offsets[id][1], 'point-label');
      });
      setStatus('BE:EC と CF:FD から、平行四辺形内の連比を描画しました。', false);
    } catch (error) {
      setStatus(error.message || '入力を確認してください。', true);
    }
  }
  ...
  function openSegmentSheet(id) {
    ...
    hint.textContent = ratioInputs[id]
      ? 'この線分は作図パラメータです。比の値は「マーク,数値」の形式で入力します。例: r,1 / s,2 / t,3'
    ...
    appendActions(function () {
      ...
      if (ratioInputs[id] && mode !== 'ratio') throw new Error(id + ' は作図パラメータなので「比の値」で入力してください。');
      if (mode === 'hidden') {
        ...
        if (ratioInputs[id]) ratioInputs[id].value = ratio.source;
      } else {
  ...
  async function saveAs(format) {
    ...
    if (format === 'png' || format === 'transparent') {
      ...
      link.download = format === 'transparent' ? 'parallelogram-ratio-transparent.png' : 'parallelogram-ratio.png';
      link.click();
      return;
    }
    ...
    pdf.save('parallelogram-ratio.pdf');
  }
  ...
  [beInput, ecInput, cfInput, fdInput].forEach(function (input) {
