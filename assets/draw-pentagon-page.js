(function () {
  ...
  function getInputValue(id) {
    return evaluateExpression(inputElements[id].value);
  }
  ...
  function normalizeAngleMarkerInput(input) {
    ...
    return /^[0-7]$/.test(value) ? Number(value) : null;
  }
  ...
  function getGeometryFromInputs() {
    ...
    return {
      ...
      baseBounds: {
        width: base.baseBounds.width * figureState.scale,
        height: base.baseBounds.height * figureState.scale
      }
    };
  }
  ...
  function isRightAngleId(id, geometry) {
    return Math.abs(getAngleMeasureDegrees(id, geometry) - 90) < 1e-4;
  }
  ...
  function getPointDefault(centroid, vertex) {
    return { x: (vertex.x * 10 - centroid.x) / 9, y: (vertex.y * 10 - centroid.y) / 9 };
  }
  ...
  function getLabelText(type, id, geometry) {
    ...
    if (type === 'angle') {
      const base = angleMode === 'degrees'
        ? (formatNumber(getAngleMeasureDegrees(id, geometry)) + '°')
        : formatNumber(getAngleMeasureDegrees(id, geometry) * Math.PI / 180);
  ...
  function getFigureSelectionAnchor() {
    ...
    return {
      ...
      width: Math.max(currentGeometry.baseBounds.width, 0.8),
      height: Math.max(currentGeometry.baseBounds.height, 0.8),
  ...
  function renderFigure(geometry) {
    const P = geometry.points;
    const points = pointIds.map(function (id) {
      return board.create('point', [P[id].x, P[id].y], { name: '', size: 3, fixed: true, strokeColor: '#111111', fillColor: '#111111' });
    });
  ...
  function drawAngleDecoration(id, geometry) {
    ...
    while (delta > Math.PI) delta -= Math.PI * 2;
    const style = getLabelStyle('angleMark', id);
  ...
  function renderLabels(geometry) {
    ...
    if (labelState.area.main) {
      createSelectableText(getLabelPosition('area', 'main', getDefaultPosition('area', 'main', geometry)), getLabelText('area', 'main', geometry), labelFontSize.area.main, { type: 'area', id: 'main' }, { color: '#25603b', threshold: 0.8 });
    }
  ...
  resetBtn.addEventListener('click', function () {
    inputElements.sideABLen.value = '5';
    inputElements.sideBCLen.value = '4.6';
