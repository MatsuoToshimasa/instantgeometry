(function () {
  ...
  function getGeometryFromInputs() {
    const radius = evaluateExpression(inputElements.radiusLen.value);
    const degrees = parseCentralAngle(inputElements.angleLen.value);
    const theta = degrees * Math.PI / 180;
    const basePoints = {
      O: { x: 0, y: 0 },
      A: { x: radius, y: 0 },
      B: { x: radius * Math.cos(theta), y: radius * Math.sin(theta) }
    };
    const transformed = transformBasePoints(basePoints);
    return {
      points: transformed.points,
      centroid: transformed.centroid,
      radius: radius * figureState.scale,
      angleDegrees: degrees,
      angleRadians: theta,
      area: 0.5 * radius * radius * theta * figureState.scale * figureState.scale,
      arcLength: radius * theta * figureState.scale,
      baseBounds: { width: radius * 2 * figureState.scale, height: radius * 2 * figureState.scale }
    };
  }
  ...
  function getDefaultPosition(type, id, geometry) {
    ...
    if (type === 'point') {
      if (id === 'O') return { x: O.x + geometry.radius * 0.18, y: O.y + geometry.radius * 0.18 };
    ...
    if (type === 'segment') {
      ...
      const nx = -dy / len;
      const ny = dx / len;
      return { x: mid.x + nx * geometry.radius * 0.12, y: mid.y + ny * geometry.radius * 0.12 };
    }
    if (type === 'angle') {
      const half = geometry.angleRadians / 2 + figureState.rotation * Math.PI / 180;
      return { x: O.x + Math.cos(half) * geometry.radius * 0.28, y: O.y + Math.sin(half) * geometry.radius * 0.28 };
    }
    const half = geometry.angleRadians / 2 + figureState.rotation * Math.PI / 180;
    return { x: O.x + Math.cos(half) * geometry.radius * 0.46, y: O.y + Math.sin(half) * geometry.radius * 0.46 };
  }
  function getLabelText(type, id, geometry) {
    ...
    if (type === 'segment') {
      ...
      return custom ? appendUnit(custom, false) : appendUnit(formatNumber(geometry.radius), false);
    }
    if (type === 'angle') {
  ...
  function pointInFigureZone(point) {
    ...
    if (r > currentGeometry.radius + 1e-8) return false;
  ...
  function renderFigure(geometry) {
    ...
    board.create('curve', [
      function (t) { return O.x + geometry.radius * Math.cos(rotationRad + t); },
      function (t) { return O.y + geometry.radius * Math.sin(rotationRad + t); },
      0, theta
    ...
    for (let i = 0; i <= 48; i += 1) {
      const angle = rotationRad + theta * (i / 48);
      sectorPoints.push([O.x + geometry.radius * Math.cos(angle), O.y + geometry.radius * Math.sin(angle)]);
    }
  ...
  function drawAngleDecoration(geometry) {
    ...
    const radius = Math.max(0.08, (geometry.radius / 10) * Math.max(0.35, Math.min(8, labelFontSize.angleMark.AOB / 26)));
  ...
  function drawRightAngleDecoration(geometry) {
    ...
    const size = Math.max(0.02, geometry.radius * 0.06 * sizeScale);
  ...
  async function handleDownload(format) {
    ...
    try {
      ...
      if (format === 'pdf') {
        ...
        link.download = format === 'png-transparent' ? 'sector-transparent.png' : 'sector.png';
        link.click();
      }
