(function () {
  ...
  const allPointIds = 'ABCDEFGHIJ'.split('');
  const shapeMeta = { name: '星型', slug: 'star' };
  const exportAspects = [
    { label: '1:1', value: 1 },
  ...
  const inputElements = Array.from(document.querySelectorAll('[data-star-input]')).reduce(function (acc, element) {
  ...
  function getPointIds() { return allPointIds.slice(); }
  ...
  function getBaseStar() {
    const side = getInputValue('sideLen');
    const tipAngleDeg = getInputValue('tipAngle');
    if (tipAngleDeg <= 0 || tipAngleDeg >= 160) throw new Error('尖り角は 0° より大きく 160° 未満で入力してください。');
    const tipAngle = tipAngleDeg * Math.PI / 180;
    const baseLen = 2 * side * Math.sin(tipAngle / 2);
    if (!(baseLen > 0)) throw new Error('星型を作れません。');
    const pentagonRadius = baseLen / (2 * Math.sin(Math.PI / 5));
    const apothem = pentagonRadius * Math.cos(Math.PI / 5);
    const spikeHeight = side * Math.cos(tipAngle / 2);
    const tipRadius = apothem + spikeHeight;
    if (!(tipRadius > 0)) throw new Error('星型を作れません。');
    const innerVertices = [];
    const outerTips = [];
    const startInner = -Math.PI / 2 - Math.PI / 5;
    const startTip = -Math.PI / 2;
    for (let index = 0; index < 5; index += 1) {
      const innerAngle = startInner + (Math.PI * 2 * index / 5);
      const tipAngleValue = startTip + (Math.PI * 2 * index / 5);
      innerVertices.push({ x: pentagonRadius * Math.cos(innerAngle), y: pentagonRadius * Math.sin(innerAngle) });
      outerTips.push({ x: tipRadius * Math.cos(tipAngleValue), y: tipRadius * Math.sin(tipAngleValue) });
    }
    const pointIds = getPointIds();
    const orderedPoints = [
      outerTips[0], innerVertices[1],
      outerTips[1], innerVertices[2],
      outerTips[2], innerVertices[3],
      outerTips[3], innerVertices[4],
      outerTips[4], innerVertices[0]
    ];
    const basePoints = {};
    pointIds.forEach(function (id, index) { basePoints[id] = orderedPoints[index]; });
    const xs = orderedPoints.map(function (p) { return p.x; });
    const ys = orderedPoints.map(function (p) { return p.y; });
    return {
      pointIds: pointIds,
  ...
  function renderLabelToggleButtons() {
    ...
    const geometryStub = { pointIds: getPointIds(), segmentIds: getSegmentIds(getPointIds()) };
  ...
  resetBtn.addEventListener('click', function () {
    inputElements.sideLen.value = '4';
    inputElements.tipAngle.value = '36';
