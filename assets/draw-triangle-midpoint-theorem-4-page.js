(function () {
  ...
  const SEGMENT_LABELS = {
    AD: { points: ['A', 'D'], color: '#2a5bd7', offset: { x: 0, y: -34 } },
    DB: { points: ['D', 'B'], color: '#2a5bd7', offset: { x: 0, y: -34 } },
    BE: { points: ['B', 'E'], color: '#2a5bd7', offset: { x: 36, y: 18 } },
    EC: { points: ['E', 'C'], color: '#2a5bd7', offset: { x: 36, y: 18 } },
    CF: { points: ['C', 'F'], color: '#2a5bd7', offset: { x: -36, y: 18 } },
    FA: { points: ['F', 'A'], color: '#2a5bd7', offset: { x: -36, y: 18 } },
    DE: { points: ['D', 'E'], color: '#2a5bd7', offset: { x: 22, y: -30 } },
    EF: { points: ['E', 'F'], color: '#2a5bd7', offset: { x: 0, y: 38 } },
    FD: { points: ['F', 'D'], color: '#2a5bd7', offset: { x: -22, y: -30 } }
  };

  const AREA_LABELS = {};

  const ANGLE_LABELS = {};
  const POINT_IDS = ['A', 'B', 'C', 'D', 'E', 'F'];

  const state = {
    pointInputs: { A: 'A', B: 'B', C: 'C', D: 'D', E: 'E', F: 'F' },
    segmentInputs: { AD: '', DB: '', BE: '', EC: '', CF: '', FA: '', DE: '', EF: '', FD: '' },
    segmentKinds: { AD: 'plain', DB: 'plain', BE: 'plain', EC: 'plain', CF: 'plain', FA: 'plain', DE: 'plain', EF: 'plain', FD: 'plain' },
    segmentArcVisible: { AD: false, DB: false, BE: false, EC: false, CF: false, FA: false, DE: false, EF: false, FD: false },
    angleInputs: {},
    angleKinds: {},
    areaInputs: {},
    areaColors: {},
  ...
  function getSegmentNumericText(id, values) {
    if (id === 'AB') return formatNumber(values.ab);
    if (id === 'BC') return formatNumber(values.bc);
    if (id === 'CA') return formatNumber(values.ca);
    if (id === 'AD' || id === 'DB') return formatNumber(values.ab / 2);
    if (id === 'BE' || id === 'EC') return formatNumber(values.bc / 2);
    if (id === 'CF' || id === 'FA') return formatNumber(values.ca / 2);
    if (id === 'DE') return formatNumber(values.ca / 2);
    if (id === 'EF') return formatNumber(values.ab / 2);
    if (id === 'FD') return formatNumber(values.bc / 2);
    return '';
  }
  ...
  function constructGeometry(ab, bc, ca) {
    if (ab + bc <= ca || bc + ca <= ab || ca + ab <= bc) {
      throw new Error('AB, BC, CA で三角形 ABC が作れる長さを入力してください。');
    }
    const ax = (ab * ab + bc * bc - ca * ca) / (2 * bc);
    const aySquared = ab * ab - ax * ax;
    if (aySquared <= 0) {
      throw new Error('AB, BC, CA で三角形 ABC が作れる長さを入力してください。');
    }
    const ay = Math.sqrt(aySquared);
    const raw = {
      A: { x: ax, y: -ay },
      B: { x: 0, y: 0 },
      C: { x: bc, y: 0 }
    };
    raw.D = midpoint(raw.A, raw.B);
    raw.E = midpoint(raw.B, raw.C);
    raw.F = midpoint(raw.C, raw.A);
    return {
  ...
  function render() {
    try {
      ...
      const geometry = constructGeometry(ab, bc, ca);
      ...
      [
        ...
        { id: 'D', point: D, label: { x: D.x - 42, y: D.y - 10 } },
        { id: 'E', point: E, label: { x: E.x, y: E.y + 48 } },
        { id: 'F', point: F, label: { x: F.x + 42, y: F.y - 10 } }
      ...
      setStatus('AB, BC, CA の中点 D, E, F をもつ三角形 ABC を描画しました。', false);
    } catch (error) {
      stage.innerHTML = '';
  ...
  async function saveAs(format) {
    ...
    if (format === 'png' || format === 'transparent') {
      ...
      link.download = format === 'transparent' ? 'triangle-midpoint-theorem-4-transparent.png' : 'triangle-midpoint-theorem-4.png';
      link.click();
      return;
    }
    ...
    pdf.save('triangle-midpoint-theorem-4.pdf');
  }
  ...
  abInput.addEventListener('input', render);
  bcInput.addEventListener('input', render);
  caInput.addEventListener('input', render);
