(function () {
  ...
  const centerA = document.getElementById('centerA');
  const centerB = document.getElementById('centerB');
  const radiusC = document.getElementById('radiusC');
  ...
  if (!stage || !centerA || !centerB || !radiusC) return;
  ...
  function parseCenterA() {
    const text = String(centerA.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('中心のx座標 a は数値で入力してください。');
    }
    return Number(text);
  }

  function parseCenterB() {
    const text = String(centerB.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('中心のy座標 b は数値で入力してください。');
    }
    return Number(text);
  }

  function parseRadiusC() {
    const text = String(radiusC.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('半径 c は数値で入力してください。');
    }
    const value = Number(text);
    if (value <= 0) throw new Error('半径 c は0より大きい数値で入力してください。');
    return value;
  }
  ...
  function shiftedTerm(variable, value) {
    if (Math.abs(value) < 1e-10) return variable + '²';
    return '(' + variable + ' ' + (value > 0 ? '- ' : '+ ') + formatNumber(Math.abs(value)) + ')²';
  }

  function formula(a, b, c) {
    return shiftedTerm('x', a) + ' + ' + shiftedTerm('y', b) + ' = ' + formatNumber(c) + '²';
  }
  ...
  function visibleCirclePoints(a, b, c) {
    const points = [];
    const steps = 240;
    for (let i = 0; i <= steps; i += 1) {
      const theta = Math.PI * 2 * (i / steps);
      const x = a + Math.cos(theta) * c;
      const y = b + Math.sin(theta) * c;
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseCenterASafe(), parseCenterBSafe(), parseRadiusCSafe()));
    if (label) {
  ...
  function drawCurveGraph(a, b, c) {
    const points = visibleCirclePoints(a, b, c);
  ...
  function render() {
    ...
    try {
      ...
      b = parseCenterB();
      c = parseRadiusC();
      setStatus('中心 (a, b) と半径 c を入力すると、円を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-conic-circle-transparent.png' : 'function-conic-circle.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-conic-circle.pdf');
    });
  }
  ...
  centerA.addEventListener('input', render);
  centerB.addEventListener('input', render);
  radiusC.addEventListener('input', render);
