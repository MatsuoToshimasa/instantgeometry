(function () {
  ...
  function parseAxisA() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('横軸半径 a は数値で入力してください。');
    }
    const value = Number(text);
    if (value <= 0) throw new Error('横軸半径 a は0より大きい数値で入力してください。');
    return value;
  }
  ...
  function parseAxisB() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('縦軸半径 b は数値で入力してください。');
    }
    const value = Number(text);
    if (value <= 0) throw new Error('縦軸半径 b は0より大きい数値で入力してください。');
    return value;
  }
  ...
  function visibleEllipsePoints(a, b) {
    const points = [];
    const steps = 240;
  ...
  function ellipseFoci(a, b) {
    ...
    if (a >= b) {
      return [
        { id: 'F1', point: point(-c, 0), labelDx: -54, labelDy: -16 },
        { id: 'F2', point: point(c, 0), labelDx: 54, labelDy: -16 }
      ];
    }
    return [
      { id: 'F1', point: point(0, -c), labelDx: 54, labelDy: -16 },
      { id: 'F2', point: point(0, c), labelDx: 54, labelDy: 24 }
    ];
  }
  ...
  function drawCurveGraph(a, b) {
    const points = visibleEllipsePoints(a, b);
  ...
  function drawPoint(id, p, options) {
    const opts = options || {};
    ...
    if (label) drawText(label, p.x + (opts.labelDx || 15), p.y + (opts.labelDy || -12), '', function () { openPointSheet(id); }, { kind: 'point', id: id });
  }

  function drawFoci(a, b) {
    ellipseFoci(a, b).forEach(function (focus) {
      drawPoint(focus.id, focus.point, { labelDx: focus.labelDx, labelDy: focus.labelDy });
    });
  }
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      b = parseAxisB();
      setStatus('横軸半径 a と縦軸半径 b を入力すると、楕円を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-conic-ellipse-transparent.png' : 'function-conic-ellipse.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-conic-ellipse.pdf');
    });
  }
  ...
  function reset() {
    axisA.value = '4';
    axisB.value = '3';
    state.pointInputs = { F1: '焦点1', F2: '焦点2' };
    state.sideInputs = { f: ' ' };
