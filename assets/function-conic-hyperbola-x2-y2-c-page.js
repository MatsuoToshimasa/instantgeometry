(function () {
  ...
  if (!stage || !axisA || !axisB || !constC) return;
  ...
  function parseAxisA() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('横軸係数 a は数値で入力してください。');
    }
    const value = Number(text);
    if (value <= 0) throw new Error('横軸係数 a は0より大きい数値で入力してください。');
    return value;
  }
  ...
  function parseAxisB() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('縦軸係数 b は数値で入力してください。');
    }
    const value = Number(text);
    if (value <= 0) throw new Error('縦軸係数 b は0より大きい数値で入力してください。');
    return value;
  }

  function parseConstC() {
    const text = String(constC.value || '').trim();
    if (text !== '1' && text !== '-1') {
      throw new Error('定数 c は 1 または -1 のみ入力できます。');
    }
    return Number(text);
  }
  ...
  function formula(a, b, c) {
    return 'x²/' + formatNumber(a) + '² - y²/' + formatNumber(b) + '² = ' + formatNumber(c);
  }
  ...
  function visibleHyperbolaBranchPoints(a, b, c, sign) {
    const points = [];
    const steps = 240;
    if (c === 1) {
      for (let i = 0; i <= steps; i += 1) {
        const y = plot.yMin + (plot.yMax - plot.yMin) * (i / steps);
        const x = sign * a * Math.sqrt(1 + (y * y) / (b * b));
        if (x >= plot.xMin - 1e-9 && x <= plot.xMax + 1e-9) points.push(point(x, y));
      }
    } else {
      for (let i = 0; i <= steps; i += 1) {
        const x = plot.xMin + (plot.xMax - plot.xMin) * (i / steps);
        const y = sign * b * Math.sqrt(1 + (x * x) / (a * a));
        if (y >= plot.yMin - 1e-9 && y <= plot.yMax + 1e-9) points.push(point(x, y));
      }
    }
    return points;
  }
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseAxisASafe(), parseAxisBSafe(), parseConstCSafe()));
    if (label) {
  ...
  function drawAsymptoteLine(slope) {
    const endpoints = visibleLineEndpoints(slope);
    const p1 = endpoints[0];
    const p2 = endpoints[1];
    stage.appendChild(svg('line', {
      x1: p1.x,
      y1: p1.y,
      x2: p2.x,
  ...
  function drawAsymptotes(a, b) {
    const slope = b / a;
    drawAsymptoteLine(slope);
    drawAsymptoteLine(-slope);
  }

  function appendCurvePath(points) {
    ...
    stage.appendChild(hit);
    return points[Math.min(points.length - 1, Math.max(0, Math.floor(points.length * 0.42)))];
  }

  function drawCurveGraph(a, b, c) {
    const positiveBranch = visibleHyperbolaBranchPoints(a, b, c, 1);
    const negativeBranch = visibleHyperbolaBranchPoints(a, b, c, -1);
    const anchor = appendCurvePath(positiveBranch) || appendCurvePath(negativeBranch);
    if (positiveBranch.length) appendCurvePath(negativeBranch);
    const label = labelFromValue(state.sideInputs.f, formula(parseAxisASafe(), parseAxisBSafe(), parseConstCSafe()));
    if (label) {
      const baseAnchor = anchor || point(0, 0);
      const base = {
        x: Math.min(plot.right - 150, Math.max(plot.left + 16, baseAnchor.x + 18)),
        y: Math.min(plot.bottom - 22, Math.max(plot.top + 34, baseAnchor.y - 20))
      };
      drawText(label, base.x, base.y, 'muted', openLineSheet, { kind: 'side', id: 'f' });
    }
  }
  ...
  function render() {
    ...
    try {
      ...
      b = parseAxisB();
      c = parseConstC();
      setStatus('係数 a, b と定数 c を入力すると、二次曲線を描画します。c は 1 または -1 のみです。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-conic-hyperbola-x2-y2-c-transparent.png' : 'function-conic-hyperbola-x2-y2-c.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-conic-hyperbola-x2-y2-c.pdf');
    });
  }
