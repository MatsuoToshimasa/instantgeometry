(function () {
  ...
  const state = {
    pointInputs: { O: '', P: '', Q: '' },
    sideInputs: { f: ' ', d: '準線' },
    sideKinds: { f: 'plain', d: 'dashed' },
    sideArcVisible: { f: false, d: false },
    angleInputs: { theta: '' },
  ...
  function parseFocusP() {
    const text = String(focusP.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('焦点距離 p は数値で入力してください。');
    }
    const value = Number(text);
    if (Math.abs(value) < 1e-10) throw new Error('焦点距離 p は0以外の数値で入力してください。');
    return value;
  }
  ...
  function verticalLineEndpoints(x) {
    return [point(x, plot.yMin), point(x, plot.yMax)];
  }

  function visibleParabolaPoints(p) {
    const points = [];
    const steps = 360;
    for (let i = 0; i <= steps; i += 1) {
      const y = plot.yMin + (plot.yMax - plot.yMin) * (i / steps);
      const x = y * y / (4 * p);
      if (x >= plot.xMin - 1e-9 && x <= plot.xMax + 1e-9) {
        points.push(point(x, y));
      }
    }
    if (!points.length) points.push(point(0, 0));
    return points;
  }
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseFocusPSafe()));
    if (label) {
  ...
  function drawDirectrix(p) {
    const x = -p;
    if (x < plot.xMin || x > plot.xMax) return;
    const endpoints = verticalLineEndpoints(x);
    const p1 = endpoints[0];
    const p2 = endpoints[1];
    const line = svg('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'function-line is-dashed' });
    stage.appendChild(line);
    const hit = svg('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'function-line-hit' });
    hit.addEventListener('click', function () {
      if (!moveMode) openDirectrixSheet();
    });
    stage.appendChild(hit);
    const label = labelFromValue(state.sideInputs.d, 'x = ' + formatNumber(-p));
    if (label) {
      const labelX = Math.min(plot.right - 72, Math.max(plot.left + 72, p2.x + (p >= 0 ? -48 : 48)));
      const base = { x: labelX, y: plot.top + 42 };
      drawText(label, base.x, base.y, 'muted', openDirectrixSheet, { kind: 'side', id: 'd' });
    }
  }

  function drawCurveGraph(p) {
    const points = visibleParabolaPoints(p);
  ...
  function render() {
    ...
    try {
      p = parseFocusP();
      setStatus('焦点距離 p を入力すると、放物線を描画します。', false);
    } catch (error) {
  ...
  function openSheetForTarget(kind, id) {
    ...
    if (kind === 'side' && id === 'd') openDirectrixSheet();
    if (kind === 'side' && id !== 'd') openLineSheet();
  ...
  function applyLabelValue(kind, id, editor, kindValue, arcVisibleValue, colorValue) {
    ...
    if (kind === 'side') {
      if (!state.sideInputs[id]) state.sideInputs[id] = '';
  ...
  function openDirectrixSheet() {
    renderCommonSheet({
      kind: 'side',
      id: 'd',
      title: '準線',
      value: state.sideInputs.d || '',
      hasNumericMode: true,
      segmentKinds: false,
      kindValue: state.sideKinds.d || 'dashed',
      arcVisible: false,
      hint: '非表示、数値、比の値、自由入力を選べます。数値では準線の式を表示します。\n' + RATIO_LABEL_HINT
    });
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-conic-parabola-y2-4px-transparent.png' : 'function-conic-parabola-y2-4px.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-conic-parabola-y2-4px.pdf');
    });
  }
  ...
  function reset() {
    ...
    state.sideInputs = { f: ' ', d: '準線' };
    state.sideKinds = { f: 'plain', d: 'dashed' };
    state.sideArcVisible = { f: false, d: false };
  ...
  focusP.addEventListener('input', render);
