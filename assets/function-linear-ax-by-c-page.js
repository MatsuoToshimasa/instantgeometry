(function () {
  ...
  if (!stage || !coefA || !coefB || !coefC) return;
  ...
  function parseIntercept() {
    return parseNumberInput(coefB, '係数 b');
  }

  function parseConstant() {
    return parseNumberInput(coefC, '定数 c');
  }
  ...
  function formatImplicitTerm(value, variable, isFirst) {
    if (Math.abs(value) < 1e-10) return '';
    const sign = value < 0 ? '-' : '+';
    const abs = Math.abs(value);
    const body = (Math.abs(abs - 1) < 1e-10 ? '' : formatNumber(abs)) + variable;
    if (isFirst) return (value < 0 ? '-' : '') + body;
    return ' ' + sign + ' ' + body;
  }

  function formula(a, b, c) {
    const xTerm = formatImplicitTerm(a, 'x', true);
    const yTerm = formatImplicitTerm(b, 'y', !xTerm);
    const left = (xTerm + yTerm) || '0';
  ...
  function visibleLineEndpoints(a, b, c) {
    if (Math.abs(a) < 1e-10 && Math.abs(b) < 1e-10) return null;
    if (Math.abs(b) < 1e-10) {
      const x = c / a;
      if (x < plot.xMin - 1e-9 || x > plot.xMax + 1e-9) return null;
      return [point(x, plot.yMin), point(x, plot.yMax)];
    }
    if (Math.abs(a) < 1e-10) {
      const y = c / b;
      if (y < plot.yMin - 1e-9 || y > plot.yMax + 1e-9) return null;
      return [point(plot.xMin, y), point(plot.xMax, y)];
    }

    const candidates = [
      { x: plot.xMin, y: (c - a * plot.xMin) / b },
      { x: plot.xMax, y: (c - a * plot.xMax) / b },
      { y: plot.yMin, x: (c - b * plot.yMin) / a },
      { y: plot.yMax, x: (c - b * plot.yMax) / a }
    ].filter(function (p) {
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseCoefficientSafe(), parseInterceptSafe(), parseConstantSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      ...
      b = parseIntercept();
      c = parseConstant();
      setStatus('係数 a、b、c を入力すると、直線を描画します。', false);
    } catch (error) {
    ...
    const endpoints = visibleLineEndpoints(a, b, c);
    if (!endpoints) {
      setStatus(Math.abs(a) < 1e-10 && Math.abs(b) < 1e-10 ? 'a と b のどちらかは 0 以外にしてください。' : '表示範囲内に直線がありません。', true);
      return;
    }
    drawLineGraph(endpoints[0], endpoints[1]);
    drawPoint('O', point(0, 0));
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-ax-by-c-transparent.png' : 'function-ax-by-c.png';
