(function () {
  ...
  function parseCoefficient() {
    const value = parseNumberInput(coefA, 'x切片 a');
    if (Math.abs(value) < 1e-10) throw new Error('x切片 a は 0 以外で入力してください。');
    return value;
  }

  function parseIntercept() {
    const value = parseNumberInput(coefB, 'y切片 b');
    if (Math.abs(value) < 1e-10) throw new Error('y切片 b は 0 以外で入力してください。');
    return value;
  }
  ...
  function formula(a, b) {
    return 'x/' + formatNumber(a) + ' + y/' + formatNumber(b) + ' = 1';
  }
  ...
  function visibleLineEndpoints(a, b) {
    ...
    const candidates = [
      { x: plot.xMin, y: b - (b / a) * plot.xMin },
      { x: plot.xMax, y: b - (b / a) * plot.xMax },
      { y: plot.yMin, x: a - (a / b) * plot.yMin },
      { y: plot.yMax, x: a - (a / b) * plot.yMax }
    ].filter(function (p) {
    ...
    if (unique.length < 2) return [point(plot.xMin, slope * plot.xMin + b), point(plot.xMax, slope * plot.xMax + b)];
  ...
  function render() {
    ...
    try {
      a = parseCoefficient();
      b = parseIntercept();
      setStatus('x切片 a と y切片 b を入力すると、直線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-intercept-form-transparent.png' : 'function-intercept-form.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-intercept-form.pdf');
    });
  }
