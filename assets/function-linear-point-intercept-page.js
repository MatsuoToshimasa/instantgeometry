(function () {
  ...
  const interceptB = document.getElementById('interceptB');
  ...
  if (!stage || !pointAX || !pointAY || !interceptB) return;
  ...
  function parseIntercept() {
    return parseNumberInput(interceptB, '切片 b');
  }
  ...
  function slopeFromPointIntercept(xA, yA, b) {
    if (Math.abs(xA) < 1e-10) {
      if (Math.abs(yA - b) < 1e-10) throw new Error('点Aのx座標が0で点Aが切片上にあるため、直線が一意に定まりません。');
      throw new Error('点Aのx座標が0のため、y切片 b と同時に通る直線を作れません。');
    }
    return (yA - b) / xA;
  }
  ...
  function formula(xA, yA, b) {
    const a = slopeFromPointIntercept(xA, yA, b);
    const slope = formatSlopeTerm(a);
    if (!slope) return 'y = ' + formatNumber(b);
    if (Math.abs(b) < 1e-10) return 'y = ' + slope;
    return 'y = ' + slope + (b > 0 ? ' + ' : ' - ') + formatNumber(Math.abs(b));
  }
  ...
  function parseInterceptSafe() {
    try {
      return parseIntercept();
    } catch (error) {
      return 0;
    }
  }
  ...
  function visibleLineEndpoints(xA, yA, b) {
    const a = slopeFromPointIntercept(xA, yA, b);
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parsePointAXSafe(), parsePointAYSafe(), parseInterceptSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      ...
      yA = parsePointAY();
      b = parseIntercept();
      endpoints = visibleLineEndpoints(xA, yA, b);
      setStatus('点Aの座標と切片 b を入力すると、直線を描画します。', false);
    } catch (error) {
  ...
  function openAngleSheet() {
    renderCommonSheet({
      ...
      angleValue: Math.abs(Math.atan(slopeFromPointIntercept(parsePointAXSafe(), parsePointAYSafe(), parseInterceptSafe())) * 180 / Math.PI),
      hint: '非表示、数値、比の値、自由入力を選べます。\n' + RATIO_LABEL_HINT
    });
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-point-intercept-transparent.png' : 'function-point-intercept.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-point-intercept.pdf');
    });
  }
  ...
  interceptB.addEventListener('input', render);
