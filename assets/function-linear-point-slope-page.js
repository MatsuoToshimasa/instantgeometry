(function () {
  ...
  if (!stage || !coefA || !pointAX || !pointAY) return;
  ...
  function parseCoefficient() {
    return parseNumberInput(coefA, '係数 a');
  }
  ...
  function formula(a, xA, yA) {
    const ySide = Math.abs(yA) < 1e-10
      ? 'y'
      : 'y ' + (yA > 0 ? '- ' : '+ ') + formatNumber(Math.abs(yA));
    const slope = Math.abs(a) < 1e-10
      ? '0'
      : (Math.abs(a - 1) < 1e-10 ? '' : (Math.abs(a + 1) < 1e-10 ? '-' : formatNumber(a))) + '(x ' + (xA >= 0 ? '- ' : '+ ') + formatNumber(Math.abs(xA)) + ')';
    return ySide + ' = ' + slope;
  }
  ...
  function visibleLineEndpoints(a, xA, yA) {
    const b = yA - a * xA;
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseCoefficientSafe(), parsePointAXSafe(), parsePointAYSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      ...
      xA = parsePointAX();
      yA = parsePointAY();
      setStatus('点Aの座標と傾き a を入力すると、直線を描画します。', false);
    } catch (error) {
    ...
    const endpoints = visibleLineEndpoints(a, xA, yA);
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-point-slope-transparent.png' : 'function-point-slope.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-point-slope.pdf');
    });
  }
