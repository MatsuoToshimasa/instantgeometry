(function () {
  ...
  const constC = document.getElementById('constC');
  const pointPX = document.getElementById('pointPX');
  const pointPY = document.getElementById('pointPY');
  ...
  if (!stage || !coefA || !coefB || !constC || !pointPX || !pointPY || !lineM) return;
  ...
  const state = {
    pointInputs: { P: 'P', A: 'A', B: 'B' },
    sideInputs: { f: ' ', g: ' ' },
    sideKinds: { f: 'plain', g: 'plain' },
    sideArcVisible: { f: false, g: false },
    angleInputs: { theta: '' },
  ...
  function applyViewRange() {
    const width = Math.max(1, Number(state.viewWidth) || 20);
    const height = Math.max(1, Number(state.viewHeight) || 20);
    const centerX = Number(state.viewCenterX) || 0;
    const centerY = Number(state.viewCenterY) || 0;
  ...
  function parseCoefficient() {
    const text = String(coefA.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('係数 a は数値で入力してください。');
    }
    const value = Number(text);
    if (Math.abs(value) < 1e-10) throw new Error('係数 a は0以外の数値で入力してください。');
    return value;
  }

  function parseCoefficientB() {
    const text = String(coefB.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('係数 b は数値で入力してください。');
    }
    return Number(text);
  }

  function parseConstantC() {
    const text = String(constC.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('定数 c は数値で入力してください。');
    }
    return Number(text);
  }

  function parsePointPX() {
    const text = String(pointPX.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('Pのx座標は数値で入力してください。');
    }
    return Number(text);
  }

  function parsePointPY() {
    const text = String(pointPY.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('Pのy座標は数値で入力してください。');
    }
    return Number(text);
  }

  function parseLineM() {
    const text = String(lineM.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('傾き m は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function parseCoefficientSafe() {
    try {
      return parseCoefficient();
    } catch (error) {
      return 1;
    }
  }

  function parseCoefficientBSafe() {
    try {
      return parseCoefficientB();
    } catch (error) {
      return 0;
    }
  }

  function parseConstantCSafe() {
    try {
      return parseConstantC();
    } catch (error) {
      return 0;
    }
  }

  function parsePointPXSafe() {
    try {
      return parsePointPX();
    } catch (error) {
      return 0;
  ...
  function areaBetweenValue(a, b, c, m, lineC, x1, x2) {
    function antiderivative(x) {
      return -a * x * x * x / 3 + (m - b) * x * x / 2 + (lineC - c) * x;
    }
    return Math.abs(antiderivative(x2) - antiderivative(x1));
  }
  ...
  function visibleLineEndpoints(b, c) {
    if (Math.abs(b) < 1e-10) return [point(plot.xMin, c), point(plot.xMax, c)];

    const candidates = [
      { x: plot.xMin, y: b * plot.xMin + c },
      { x: plot.xMax, y: b * plot.xMax + c },
      { y: plot.yMin, x: (plot.yMin - c) / b },
      { y: plot.yMax, x: (plot.yMax - c) / b }
    ].filter(function (p) {
    ...
    if (unique.length < 2) return [point(plot.xMin, b * plot.xMin + c), point(plot.xMax, b * plot.xMax + c)];
    return [point(unique[0].x, unique[0].y), point(unique[1].x, unique[1].y)];
  }

  function visibleParabolaPoints(a, b, c) {
    const points = [];
    const steps = 360;
    for (let i = 0; i <= steps; i += 1) {
      const x = plot.xMin + (plot.xMax - plot.xMin) * (i / steps);
      const y = a * x * x + b * x + c;
      if (y >= plot.yMin - 1e-9 && y <= plot.yMax + 1e-9) {
        points.push(point(x, y));
      }
    }
    if (!points.length) {
      const x = -b / (2 * a);
      points.push(point(x, a * x * x + b * x + c));
    }
    return points;
  }

  function intersectionPoints(a, b, c, m, lineC) {
    const linear = b - m;
    const constant = c - lineC;
    const discriminant = linear * linear - 4 * a * constant;
    ...
    const x1 = (-linear - root) / (2 * a);
    const x2 = (-linear + root) / (2 * a);
    return [point(x1, a * x1 * x1 + b * x1 + c), point(x2, a * x2 * x2 + b * x2 + c)];
  }
  ...
  function drawArea(a, b, c, m, lineC, p1, p2) {
    if (state.areaValue === '') return;
    const x1 = Math.min(p1.ux, p2.ux);
    const x2 = Math.max(p1.ux, p2.ux);
    const points = [];
    const steps = 96;
    for (let i = 0; i <= steps; i += 1) {
    ...
    for (let i = steps; i >= 0; i -= 1) {
      const x = x1 + (x2 - x1) * (i / steps);
      points.push(point(x, m * x + lineC));
    }
    const polygon = svg('polygon', {
      points: points.map(function (p) { return formatNumber(p.x) + ',' + formatNumber(p.y); }).join(' '),
      class: 'function-area'
    });
    ...
    const midX = (x1 + x2) / 2;
    const curveMid = a * midX * midX + b * midX + c;
    const lineMid = m * midX + lineC;
    const label = labelFromValue(state.areaValue, formatNumber(areaBetweenValue(a, b, c, m, lineC, x1, x2)));
    if (label) drawText(label, sx(midX), sy((curveMid + lineMid) / 2), 'muted', openAreaSheet, { kind: 'area', id: 'main' });
  }

  function drawLineGraph(p1, p2, b, c) {
    const line = svg('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'function-line' });
    line.style.stroke = '#2e7d32';
    stage.appendChild(line);
    const kind = state.sideKinds.g || 'plain';
    drawSegmentKind(kind, p1, p2);
    const hit = svg('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'function-line-hit' });
    hit.addEventListener('click', function () {
      if (!moveMode) openLineSheet('g');
    });
    stage.appendChild(hit);
    const label = labelFromValue(state.sideInputs.g, lineFormula(b, c));
    if (label) {
      const base = {
        x: Math.min(plot.right - 120, Math.max(plot.left + 16, p2.x - 90)),
        y: Math.min(plot.bottom - 22, Math.max(plot.top + 34, p2.y - 20))
      };
      const pos = getLabelPosition('side', 'g', base);
      if (state.sideArcVisible.g !== false) drawSideLabelArc(p1, p2, point(0, 0), pos);
      drawText(label, base.x, base.y, 'muted', function () { openLineSheet('g'); }, { kind: 'side', id: 'g' });
    }
  }

  function drawCurveGraph(a, b, c) {
    const points = visibleParabolaPoints(a, b, c);
    ...
    hit.addEventListener('click', function () {
      if (!moveMode) openLineSheet('f');
    });
    stage.appendChild(hit);
    const label = labelFromValue(state.sideInputs.f, quadraticFormula(parseCoefficientSafe(), parseCoefficientBSafe(), parseConstantCSafe()));
    if (label) {
      ...
      drawText(label, base.x, base.y, 'muted', function () { openLineSheet('f'); }, { kind: 'side', id: 'f' });
    }
  }
  ...
  function render() {
    ...
    try {
      ...
      py = parsePointPY();
      m = parseLineM();
      const curveAtP = a * px * px + b * px + c;
      if (py <= curveAtP + 1e-10) {
        throw new Error('点Pは二次関数より上の領域に置いてください。');
      }
      lineC = py - m * px;
      intersections = intersectionPoints(a, b, c, m, lineC);
      if (intersections.length !== 2) {
        throw new Error('一次関数と二次関数が2点で交わるように入力してください。');
      }
      setStatus('点Pを通る直線 ' + lineFormula(m, lineC) + ' と二次関数で囲まれる面積を描画します。', false);
    } catch (error) {
    ...
    if (intersections.length === 2) drawArea(a, b, c, m, lineC, intersections[0], intersections[1]);
    drawCurveGraph(a, b, c);
    const lineEndpoints = visibleLineEndpoints(m, lineC);
    drawLineGraph(lineEndpoints[0], lineEndpoints[1], m, lineC);
    drawPoint('P', point(px, py));
    intersections.forEach(function (p, index) {
      if (p.ux >= plot.xMin - 1e-9 && p.ux <= plot.xMax + 1e-9 && p.uy >= plot.yMin - 1e-9 && p.uy <= plot.yMax + 1e-9) {
        drawPoint(index === 0 ? 'A' : 'B', p);
      }
    });
  }
  ...
  function parseSheetNumber(input, labelText, positive) {
  ...
  function applyLabelValue(kind, id, editor, kindValue, arcVisibleValue, colorValue) {
    ...
    if (kind === 'side') {
      ...
      state.sideInputs[id] = labelValue();
      if (mode === 'hidden') state.sideArcVisible[id] = false;
      return;
    }
    if (kind === 'angle') {
  ...
  function openLineSheet(id) {
    ...
    renderCommonSheet({
      kind: 'side',
      id: targetId,
      title: targetId === 'g' ? '点Pを通る直線' : '二次関数グラフ',
      value: state.sideInputs[targetId] || '',
      hasNumericMode: true,
      segmentKinds: targetId === 'g',
      kindValue: state.sideKinds[targetId] || 'plain',
      arcVisible: state.sideArcVisible[targetId] !== false,
  ...
  function openAngleSheet() {
    renderCommonSheet({
      ...
      angleValue: Math.abs(Math.atan(parseCoefficientSafe()) * 180 / Math.PI),
      hint: '非表示、数値、比の値、自由入力を選べます。\n' + RATIO_LABEL_HINT
    });
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-linear-quadratic-enclosed-area-transparent.png' : 'function-linear-quadratic-enclosed-area.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-linear-quadratic-enclosed-area.pdf');
    });
  }
  ...
  function reset() {
    ...
    pointPY.value = '6';
    lineM.value = '0.5';
    state.pointInputs = { P: 'P', A: 'A', B: 'B' };
    state.sideInputs = { f: ' ', g: ' ' };
    state.sideKinds = { f: 'plain', g: 'plain' };
    state.sideArcVisible = { f: false, g: false };
    state.angleInputs = { theta: '' };
    state.angleKinds = { theta: 'hidden' };
    state.areaValue = ' ';
    state.areaColor = '#2a5bd7';
    state.tickLabelInterval = 2;
  ...
  coefA.addEventListener('input', render);
  coefB.addEventListener('input', render);
  constC.addEventListener('input', render);
  pointPX.addEventListener('input', render);
  pointPY.addEventListener('input', render);
  lineM.addEventListener('input', render);
