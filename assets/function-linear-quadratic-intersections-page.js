(function () {
  ...
  if (!stage || !coefA || !coefB || !constC) return;
  ...
  function parseCoefficientB() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数の係数 b は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function parseConstantC() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数の切片 c は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function render() {
    ...
    try {
      ...
      b = parseCoefficientB();
      c = parseConstantC();
      setStatus('二次関数の係数 a、一次関数の係数 b、一次関数の切片 c を入力すると、2つのグラフを描画します。', false);
    } catch (error) {
    ...
    drawCurveGraph(a);
    const lineEndpoints = visibleLineEndpoints(b, c);
    drawLineGraph(lineEndpoints[0], lineEndpoints[1], b, c);
  ...
  function openLineSheet(id) {
    ...
    renderCommonSheet({
      kind: 'side',
      id: targetId,
      title: targetId === 'g' ? '一次関数グラフ' : '二次関数グラフ',
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-linear-quadratic-intersections-transparent.png' : 'function-linear-quadratic-intersections.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-linear-quadratic-intersections.pdf');
    });
  }
