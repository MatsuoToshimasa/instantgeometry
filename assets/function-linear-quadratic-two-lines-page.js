(function () {
  ...
  if (!stage || !coefA || !coefB || !constC || !coefD || !constE) return;
  ...
  const state = {
    ...
    sideInputs: { f: ' ', g: ' ', h: ' ' },
    sideKinds: { f: 'plain', g: 'plain', h: 'plain' },
    sideArcVisible: { f: false, g: false, h: false },
    angleInputs: { theta: '' },
  ...
  function parseCoefficient() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('二次関数の係数 a は数値で入力してください。');
    }
    const value = Number(text);
    if (Math.abs(value) < 1e-10) throw new Error('二次関数の係数 a は0以外の数値で入力してください。');
    return value;
  }
  ...
  function parseCoefficientB() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数①の係数 b は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function parseConstantC() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数①の切片 c は数値で入力してください。');
    }
    return Number(text);
  }

  function parseCoefficientD() {
    const text = String(coefD.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数②の係数 d は数値で入力してください。');
    }
    return Number(text);
  }

  function parseConstantE() {
    const text = String(constE.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数②の切片 e は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function parseConstantESafe() {
    try {
      return parseConstantE();
    } catch (error) {
      return 6;
    }
  }
  ...
  function render() {
    ...
    try {
      ...
      d = parseCoefficientD();
      e = parseConstantE();
      setStatus('二次関数の係数 a、一次関数①の係数 b・切片 c、一次関数②の係数 d・切片 e を入力すると、3つのグラフを描画します。', false);
    } catch (error) {
    ...
    drawCurveGraph(a);
    const lineEndpoints1 = visibleLineEndpoints(b, c);
    drawLineGraph('g', lineEndpoints1[0], lineEndpoints1[1], b, c, '#2e7d32');
    const lineEndpoints2 = visibleLineEndpoints(d, e);
    drawLineGraph('h', lineEndpoints2[0], lineEndpoints2[1], d, e, '#8e44ad');
    intersectionPoints(a, b, c).forEach(function (p, index) {
      if (p.ux >= plot.xMin - 1e-9 && p.ux <= plot.xMax + 1e-9 && p.uy >= plot.yMin - 1e-9 && p.uy <= plot.yMax + 1e-9) {
        drawPoint(index === 0 ? 'A' : 'B', p);
      }
    });
    intersectionPoints(a, d, e).forEach(function (p, index) {
      if (p.ux >= plot.xMin - 1e-9 && p.ux <= plot.xMax + 1e-9 && p.uy >= plot.yMin - 1e-9 && p.uy <= plot.yMax + 1e-9) {
        drawPoint(index === 0 ? 'C' : 'D', p);
      }
    });
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-linear-quadratic-two-lines-transparent.png' : 'function-linear-quadratic-two-lines.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-linear-quadratic-two-lines.pdf');
    });
  }
  ...
  function reset() {
    ...
    coefD.value = '-1';
    constE.value = '6';
    state.pointInputs = { A: 'A', B: 'B', C: 'C', D: 'D' };
    state.sideInputs = { f: ' ', g: ' ', h: ' ' };
    state.sideKinds = { f: 'plain', g: 'plain', h: 'plain' };
    state.sideArcVisible = { f: false, g: false, h: false };
  ...
  coefD.addEventListener('input', render);
  constE.addEventListener('input', render);
