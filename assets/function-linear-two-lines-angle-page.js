(function () {
  ...
  const constB = document.getElementById('constB');
  const coefC = document.getElementById('coefC');
  const constD = document.getElementById('constD');
  ...
  if (!stage || !coefA || !constB || !coefC || !constD) return;
  ...
  const state = {
    pointInputs: { P: '' },
    sideInputs: { g: ' ', h: ' ' },
    sideKinds: { g: 'plain', h: 'plain' },
    sideArcVisible: { g: false, h: false },
    angleInputs: { theta: ' ' },
    angleKinds: { theta: 'plain' },
    areaValue: '',
    areaColor: '#2a5bd7',
    tickLabelInterval: 2,
    viewCenterX: 0,
  ...
  function parseCoefficient() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数①の係数 a は数値で入力してください。');
    }
    return Number(text);
  }

  function parseCoefficientB() {
    const text = String(constB.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数①の切片 b は数値で入力してください。');
    }
    return Number(text);
  }

  function parseConstantC() {
    const text = String(coefC.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数②の係数 c は数値で入力してください。');
    }
    return Number(text);
  }

  function parseCoefficientD() {
    const text = String(constD.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('一次関数②の切片 d は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function parseCoefficientDSafe() {
    try {
      return parseCoefficientD();
    } catch (error) {
      return -1;
    }
  }
  ...
  function visibleParabolaPoints(a) {
    const points = [];
    const steps = 360;
  ...
  function intersectionPoints(a, b, c) {
    ...
    if (Math.abs(discriminant) < 1e-10) {
      const x = b / (2 * a);
      return [point(x, a * x * x)];
    }
    const root = Math.sqrt(discriminant);
    const x1 = (b - root) / (2 * a);
    const x2 = (b + root) / (2 * a);
    return [point(x1, a * x1 * x1), point(x2, a * x2 * x2)];
  }
  ...
  function drawLineGraph(id, p1, p2, b, c, color) {
    const line = svg('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'function-line' });
    line.style.stroke = color;
    stage.appendChild(line);
    const kind = state.sideKinds[id] || 'plain';
    drawSegmentKind(kind, p1, p2);
    const hit = svg('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'function-line-hit' });
    hit.addEventListener('click', function () {
      if (!moveMode) openLineSheet(id);
    });
    stage.appendChild(hit);
    const label = labelFromValue(state.sideInputs[id], lineFormula(b, c));
    if (label) {
      const base = {
        x: Math.min(plot.right - 120, Math.max(plot.left + 16, p2.x - 90)),
        y: Math.min(plot.bottom - 22, Math.max(plot.top + 34, p2.y - 20))
      };
      const pos = getLabelPosition('side', id, base);
      if (state.sideArcVisible[id] !== false) drawSideLabelArc(p1, p2, point(0, 0), pos);
      drawText(label, base.x, base.y, 'muted', function () { openLineSheet(id); }, { kind: 'side', id: id });
    }
  }

  function drawCurveGraph(a) {
    const points = visibleParabolaPoints(a);
    ...
    stage.appendChild(hit);
    const label = labelFromValue(state.sideInputs.f, quadraticFormula(parseCoefficientSafe()));
    if (label) {
  ...
  function smallAngleDelta(m1, m2) {
    let delta = Math.atan(m2) - Math.atan(m1);
    while (delta > Math.PI / 2) delta -= Math.PI;
    while (delta < -Math.PI / 2) delta += Math.PI;
    return delta;
  }

  function lineIntersection(m1, n1, m2, n2) {
    if (Math.abs(m1 - m2) < 1e-10) return null;
    const x = (n2 - n1) / (m1 - m2);
    return point(x, m1 * x + n1);
  }

  function drawTwoLineAngle(m1, n1, m2, n2) {
    const delta = smallAngleDelta(m1, m2);
    const theta = Math.abs(delta);
    const thetaText = formatNumber(theta * 180 / Math.PI) + '°';
    const intersection = lineIntersection(m1, n1, m2, n2);
    if (!intersection || intersection.ux < plot.xMin || intersection.ux > plot.xMax || intersection.uy < plot.yMin || intersection.uy > plot.yMax) {
      const fallbackLabel = labelFromValue(state.angleInputs.theta, thetaText);
      if (fallbackLabel) drawText(fallbackLabel, sx(0) + 72, sy(0) - 42, 'muted', openAngleSheet, { kind: 'angle', id: 'theta' });
      return;
    }

    if (theta > 1e-6 && state.angleKinds.theta !== 'hidden') {
      const radius = 88;
      const startAngle = Math.atan(m1);
      const endAngle = startAngle + delta;
      const start = { x: intersection.x + Math.cos(startAngle) * radius, y: intersection.y - Math.sin(startAngle) * radius };
      const end = { x: intersection.x + Math.cos(endAngle) * radius, y: intersection.y - Math.sin(endAngle) * radius };
      const d = 'M ' + start.x + ' ' + start.y + ' A ' + radius + ' ' + radius + ' 0 0 ' + (delta > 0 ? 0 : 1) + ' ' + end.x + ' ' + end.y;
      stage.appendChild(svg('path', { d: d, class: 'function-angle' }));
      const hit = svg('path', { d: d, class: 'function-angle-hit' });
      hit.addEventListener('click', function () {
        if (!moveMode) openAngleSheet();
    ...
    const label = labelFromValue(state.angleInputs.theta, thetaText);
    if (label) {
      const midAngle = Math.atan(m1) + delta / 2;
      drawText(label, intersection.x + Math.cos(midAngle) * 126, intersection.y - Math.sin(midAngle) * 126, 'muted', openAngleSheet, { kind: 'angle', id: 'theta' });
    }
  }
  ...
  function render() {
    ...
    try {
      ...
      c = parseConstantC();
      d = parseCoefficientD();
      setStatus('一次関数① y = ax + b と一次関数② y = cx + d を入力すると、2直線のなす角の小さい方をθで表示します。', false);
    } catch (error) {
    ...
    const lineEndpoints1 = visibleLineEndpoints(a, b);
    drawLineGraph('g', lineEndpoints1[0], lineEndpoints1[1], a, b, '#2e7d32');
    const lineEndpoints2 = visibleLineEndpoints(c, d);
    drawLineGraph('h', lineEndpoints2[0], lineEndpoints2[1], c, d, '#8e44ad');
    drawTwoLineAngle(a, b, c, d);
  }
  ...
  function openLineSheet(id) {
    ...
    renderCommonSheet({
      kind: 'side',
      id: targetId,
      title: targetId === 'g' ? '一次関数①グラフ' : (targetId === 'h' ? '一次関数②グラフ' : '二次関数グラフ'),
      value: state.sideInputs[targetId] || '',
      hasNumericMode: true,
      segmentKinds: targetId === 'g' || targetId === 'h',
  ...
  function openAngleSheet() {
    renderCommonSheet({
      ...
      angleValue: Math.abs(smallAngleDelta(parseCoefficientSafe(), parseConstantCSafe()) * 180 / Math.PI),
      hint: '非表示、数値、比の値、自由入力を選べます。\n' + RATIO_LABEL_HINT
    });
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-linear-two-lines-angle-transparent.png' : 'function-linear-two-lines-angle.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-linear-two-lines-angle.pdf');
    });
  }
  ...
  function reset() {
    ...
    constD.value = '0';
    state.pointInputs = { P: '' };
    state.sideInputs = { g: ' ', h: ' ' };
    state.sideKinds = { g: 'plain', h: 'plain' };
    state.sideArcVisible = { g: false, h: false };
    state.angleInputs = { theta: ' ' };
    state.angleKinds = { theta: 'plain' };
    state.areaValue = '';
  ...
  constB.addEventListener('input', render);
  coefC.addEventListener('input', render);
  constD.addEventListener('input', render);
