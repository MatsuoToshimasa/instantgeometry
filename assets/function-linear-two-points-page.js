(function () {
  ...
  const pointAX = document.getElementById('pointAX');
  const pointAY = document.getElementById('pointAY');
  const pointBX = document.getElementById('pointBX');
  const pointBY = document.getElementById('pointBY');
  ...
  if (!stage || !pointAX || !pointAY || !pointBX || !pointBY) return;
  ...
  function parsePointAX() {
    return parseNumberInput(pointAX, '点Aのx座標');
  }

  function parsePointAY() {
    return parseNumberInput(pointAY, '点Aのy座標');
  }

  function parsePointBX() {
    return parseNumberInput(pointBX, '点Bのx座標');
  }

  function parsePointBY() {
    return parseNumberInput(pointBY, '点Bのy座標');
  }
  ...
  function formatSlopeTerm(a) {
    if (Math.abs(a) < 1e-10) return '';
    if (Math.abs(a - 1) < 1e-10) return 'x';
    if (Math.abs(a + 1) < 1e-10) return '-x';
    return formatNumber(a) + 'x';
  }

  function slopeFromTwoPoints(xA, yA, xB, yB) {
    if (Math.abs(xA - xB) < 1e-10) {
      if (Math.abs(yA - yB) < 1e-10) throw new Error('点Aと点Bが同じため、直線が一意に定まりません。');
      return null;
    }
    return (yB - yA) / (xB - xA);
  }

  function formula(xA, yA, xB, yB) {
    const slope = slopeFromTwoPoints(xA, yA, xB, yB);
    if (slope === null) return 'x = ' + formatNumber(xA);
    const ySide = Math.abs(yA) < 1e-10 ? 'y' : 'y ' + (yA > 0 ? '- ' : '+ ') + formatNumber(Math.abs(yA));
    const right = Math.abs(slope) < 1e-10
      ? '0'
      : (Math.abs(slope - 1) < 1e-10 ? '' : (Math.abs(slope + 1) < 1e-10 ? '-' : formatNumber(slope))) + '(x ' + (xA >= 0 ? '- ' : '+ ') + formatNumber(Math.abs(xA)) + ')';
    return ySide + ' = ' + right;
  }
  ...
  function visibleLineEndpoints(xA, yA, xB, yB) {
    const a = slopeFromTwoPoints(xA, yA, xB, yB);
    if (a === null) return [point(xA, plot.yMin), point(xA, plot.yMax)];
    const b = yA - a * xA;
    if (Math.abs(a) < 1e-10) return [point(plot.xMin, yA), point(plot.xMax, yA)];

    const candidates = [
      { x: plot.xMin, y: a * plot.xMin + b },
      { x: plot.xMax, y: a * plot.xMax + b },
      { y: plot.yMin, x: (plot.yMin - b) / a },
      { y: plot.yMax, x: (plot.yMax - b) / a }
    ].filter(function (p) {
    ...
    if (unique.length < 2) return [point(plot.xMin, a * plot.xMin + b), point(plot.xMax, a * plot.xMax + b)];
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parsePointAXSafe(), parsePointAYSafe(), parsePointBXSafe(), parsePointBYSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      ...
      xB = parsePointBX();
      yB = parsePointBY();
      endpoints = visibleLineEndpoints(xA, yA, xB, yB);
      setStatus('点Aと点Bの座標を入力すると、直線を描画します。', false);
    } catch (error) {
      ...
      try {
        ...
        xB = parsePointBX();
        yB = parsePointBY();
        drawPoint('A', point(xA, yA));
        drawPoint('B', point(xB, yB));
      } catch (ignored) {}
      return;
    }
    drawLineGraph(endpoints[0], endpoints[1]);
    drawPoint('A', point(xA, yA));
    drawPoint('B', point(xB, yB));
    drawPoint('P', endpoints[0]);
    drawPoint('Q', endpoints[1]);
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-two-points-transparent.png' : 'function-two-points.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-two-points.pdf');
    });
  }
  ...
  pointAX.addEventListener('input', render);
  pointAY.addEventListener('input', render);
  pointBX.addEventListener('input', render);
  pointBY.addEventListener('input', render);
