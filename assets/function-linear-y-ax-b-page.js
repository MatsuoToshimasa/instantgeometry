(function () {
  ...
  function parseIntercept() {
    return parseNumberInput(coefB, '切片 b');
  }
  ...
  function visibleLineEndpoints(a, b) {
    if (Math.abs(a) < 1e-10) return [point(plot.xMin, b), point(plot.xMax, b)];
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseCoefficientSafe(), parseInterceptSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      a = parseCoefficient();
      b = parseIntercept();
      setStatus('係数 a と切片 b を入力すると、直線を描画します。', false);
    } catch (error) {
      setStatus(error.message, true);
      a = 1;
      b = 1;
    }
    const endpoints = visibleLineEndpoints(a, b);
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-y-ax-b-transparent.png' : 'function-y-ax-b.png';
