(function () {
  ...
  function render() {
    ...
    try {
      a = parseCoefficient();
      setStatus('係数 a を入力すると、原点を通る直線を描画します。', false);
    } catch (error) {
      setStatus(error.message, true);
      a = 1;
    }
    const endpoints = visibleLineEndpoints(a);
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-y-ax-transparent.png' : 'function-y-ax.png';
