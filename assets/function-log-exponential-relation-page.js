(function () {
  ...
  function parseAxisA() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('底 a は数値で入力してください。');
    }
    const value = Number(text);
    if (value <= 0 || Math.abs(value - 1) < 1e-10) throw new Error('底 a は0より大きく、1以外の数値で入力してください。');
    return value;
  }
  ...
  function logFormula(a) {
    return 'y = log_' + formatNumber(a) + ' x';
  }
  ...
  function visibleLogPoints(a) {
    const points = [];
    const steps = 420;
    const xMin = 0.05;
    for (let i = 0; i <= steps; i += 1) {
      const t = i / steps;
      const x = xMin * Math.pow(plot.xMax / xMin, t);
  ...
  function visibleExponentialPoints(a) {
    const points = [];
    const steps = 420;
    for (let i = 0; i <= steps; i += 1) {
  ...
  function drawLineGraph(id, p1, p2, labelText, color) {
    ...
    const label = labelFromValue(state.sideInputs[id], labelText);
    if (label) {
  ...
  function drawCurveGraph(id, points, labelText, color) {
    if (!points.length) return;
    const d = pathFromPlotPoints(points);
    const curve = svg('path', { d: d, class: 'function-curve' });
    if (color) curve.style.stroke = color;
  ...
  function drawSymmetryLine() {
    const p1 = point(plot.xMin, plot.xMin);
    const p2 = point(plot.xMax, plot.xMax);
    const line = svg('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'function-line' });
    line.style.stroke = '#8a94a6';
    line.style.strokeWidth = '3';
    line.style.strokeDasharray = '9 8';
    stage.appendChild(line);
    drawText('y = x', sx(6.2), sy(6.2) - 18, 'muted');
  }
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      setStatus('底 a を入力すると、y = logₐ x と y = aˣ を描画します。', false);
    } catch (error) {
      setStatus(error.message, true);
      a = 2;
    }
    drawSymmetryLine();
    drawCurveGraph('f', visibleLogPoints(a), logFormula(a), '#2a5bd7');
    drawCurveGraph('g', visibleExponentialPoints(a), exponentialFormula(a), '#2e7d32');
  }
  ...
  function openLineSheet(id) {
    ...
    renderCommonSheet({
      kind: 'side',
      id: targetId,
      title: targetId === 'g' ? '指数関数グラフ' : '対数関数グラフ',
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-log-exponential-relation-transparent.png' : 'function-log-exponential-relation.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-log-exponential-relation.pdf');
    });
  }
