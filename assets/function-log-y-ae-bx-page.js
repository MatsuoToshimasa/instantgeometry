(function () {
  ...
  function exponentText(b) {
    if (Math.abs(b - 1) < 1e-10) return 'x';
    if (Math.abs(b + 1) < 1e-10) return '-x';
    return formatNumber(b) + 'x';
  }

  function formula(a, b) {
    const exponent = exponentText(b);
    if (Math.abs(a - 1) < 1e-10) return 'y = e^(' + exponent + ')';
    if (Math.abs(a + 1) < 1e-10) return 'y = -e^(' + exponent + ')';
    return 'y = ' + formatNumber(a) + 'e^(' + exponent + ')';
  }
  ...
  function visibleExponentialPoints(a, b) {
    const points = [];
    const steps = 720;
  ...
  function drawCurveGraph(a, b) {
    const points = visibleExponentialPoints(a, b);
    if (!points.length) return;
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      b = parseAxisB();
      setStatus('パラメータ a と b を入力すると、指数関数を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-log-y-ae-bx-transparent.png' : 'function-log-y-ae-bx.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-log-y-ae-bx.pdf');
    });
  }
