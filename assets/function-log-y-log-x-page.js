(function () {
  ...
  function visibleLogPoints(a) {
    const points = [];
    const steps = 560;
    const lowerVisibleX = Math.min(Math.pow(a, plot.yMin), Math.pow(a, plot.yMax));
    const upperVisibleX = Math.max(Math.pow(a, plot.yMin), Math.pow(a, plot.yMax));
    const xMin = Math.max(Number.MIN_VALUE, Math.min(plot.xMax, Math.max(0, lowerVisibleX)));
    const xMax = Math.min(plot.xMax, upperVisibleX);
    if (!Number.isFinite(xMin) || !Number.isFinite(xMax) || xMin <= 0 || xMin >= xMax) return points;
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseAxisASafe()));
    if (label) {
  ...
  function drawCurveGraph(a) {
    const points = visibleLogPoints(a);
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      setStatus('底 a を入力すると、対数関数を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-log-y-log-x-transparent.png' : 'function-log-y-log-x.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-log-y-log-x.pdf');
    });
  }
