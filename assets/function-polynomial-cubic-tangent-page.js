(function () {
  ...
  const tangentT = document.getElementById('tangentT');
  ...
  if (!stage || !coefA || !coefB || !constC || !coefD || !tangentT) return;
  ...
  function parseConstantC() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('係数 c は数値で入力してください。');
    }
    return Number(text);
  }

  function parseConstantD() {
    const text = String(coefD.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('定数 d は数値で入力してください。');
    }
    return Number(text);
  }

  function parseTangentT() {
    const text = String(tangentT.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('接点のx座標 t は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function tangentLine(a, b, c, d, t) {
    const y = a * t * t * t + b * t * t + c * t + d;
  ...
  function parseConstantDSafe() {
    try {
      return parseConstantD();
    } catch (error) {
      return 0;
    }
  }

  function parseTangentTSafe() {
    try {
      return parseTangentT();
    } catch (error) {
      return 2;
    }
  }
  ...
  function visibleCubicPoints(a, b, c, d) {
    const points = [];
    const steps = 720;
  ...
  function drawCurveGraph(a, b, c, dValue) {
    const points = visibleCubicPoints(a, b, c, dValue);
    ...
    stage.appendChild(hit);
    const label = labelFromValue(state.sideInputs.f, cubicFormula(parseCoefficientSafe(), parseCoefficientBSafe(), parseConstantCSafe(), parseConstantDSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      ...
      d = parseConstantD();
      t = parseTangentT();
      const tangent = tangentLine(a, b, c, d, t);
      setStatus('x = ' + formatNumber(t) + ' で接する接線: ' + lineFormula(tangent.slope, tangent.intercept), false);
    } catch (error) {
    ...
    const tangent = tangentLine(a, b, c, d, t);
    drawCurveGraph(a, b, c, d);
    const lineEndpoints = visibleLineEndpoints(tangent.slope, tangent.intercept);
    drawLineGraph(lineEndpoints[0], lineEndpoints[1], tangent.slope, tangent.intercept);
    if (tangent.touch.ux >= plot.xMin - 1e-9 && tangent.touch.ux <= plot.xMax + 1e-9 && tangent.touch.uy >= plot.yMin - 1e-9 && tangent.touch.uy <= plot.yMax + 1e-9) {
      drawPoint('T', tangent.touch);
    }
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-polynomial-cubic-tangent-transparent.png' : 'function-polynomial-cubic-tangent.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-polynomial-cubic-tangent.pdf');
    });
  }
  ...
  tangentT.addEventListener('input', render);
