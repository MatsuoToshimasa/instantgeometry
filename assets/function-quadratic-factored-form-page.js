(function () {
  ...
  const rootAlpha = document.getElementById('rootAlpha');
  const rootBeta = document.getElementById('rootBeta');
  ...
  if (!stage || !coefA || !rootAlpha || !rootBeta) return;
  ...
  function parseRootAlpha() {
    const text = String(rootAlpha.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('x軸との交点 α は数値で入力してください。');
    }
    return Number(text);
  }

  function parseRootBeta() {
    const text = String(rootBeta.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('x軸との交点 β は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function factorText(value) {
    ...
    return 'x ' + (value > 0 ? '- ' : '+ ') + formatNumber(Math.abs(value));
  }

  function formula(a, alpha, beta) {
    if (Math.abs(a) < 1e-10) return 'y = 0';
    const coefficient = Math.abs(a - 1) < 1e-10 ? '' : (Math.abs(a + 1) < 1e-10 ? '-' : formatNumber(a));
    return 'y = ' + coefficient + '(' + factorText(alpha) + ')(' + factorText(beta) + ')';
  }
  ...
  function parseRootAlphaSafe() {
    try {
      return parseRootAlpha();
    } catch (error) {
      return -2;
    }
  }

  function parseRootBetaSafe() {
    try {
      return parseRootBeta();
    } catch (error) {
      return 2;
    }
  }
  ...
  function visibleParabolaPoints(a, alpha, beta) {
    const points = [];
    const steps = 360;
    ...
    if (!points.length) {
      const x = (alpha + beta) / 2;
      points.push(point(x, a * (x - alpha) * (x - beta)));
    }
    return points;
  }
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseCoefficientSafe(), parseRootAlphaSafe(), parseRootBetaSafe()));
    if (label) {
  ...
  function drawCurveGraph(a, alpha, beta) {
    const points = visibleParabolaPoints(a, alpha, beta);
  ...
  function render() {
    ...
    try {
      a = parseCoefficient();
      alpha = parseRootAlpha();
      beta = parseRootBeta();
      setStatus('係数 a と x軸との交点 α, β を入力すると、放物線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-factored-form-transparent.png' : 'function-factored-form.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-factored-form.pdf');
    });
  }
  ...
  rootAlpha.addEventListener('input', render);
  rootBeta.addEventListener('input', render);
