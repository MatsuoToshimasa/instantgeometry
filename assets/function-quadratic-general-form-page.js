(function () {
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseCoefficientSafe(), parseCoefficientBSafe(), parseConstantCSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      ...
      b = parseCoefficientB();
      c = parseConstantC();
      setStatus('係数 a、係数 b、定数 c を入力すると、放物線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-general-form-transparent.png' : 'function-general-form.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-general-form.pdf');
    });
  }
