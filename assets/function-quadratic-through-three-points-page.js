(function () {
  ...
  const pointQX = document.getElementById('pointQX');
  const pointQY = document.getElementById('pointQY');
  const pointRX = document.getElementById('pointRX');
  const pointRY = document.getElementById('pointRY');
  ...
  if (!stage || !pointPX || !pointPY || !pointQX || !pointQY || !pointRX || !pointRY) return;
  ...
  function parseNumberInput(input, label) {
  ...
  function parsePointInputs() {
    const p = { id: 'P', x: parseNumberInput(pointPX, 'Pのx座標'), y: parseNumberInput(pointPY, 'Pのy座標') };
    const q = { id: 'Q', x: parseNumberInput(pointQX, 'Qのx座標'), y: parseNumberInput(pointQY, 'Qのy座標') };
    const r = { id: 'R', x: parseNumberInput(pointRX, 'Rのx座標'), y: parseNumberInput(pointRY, 'Rのy座標') };
    if (Math.abs(p.x - q.x) < 1e-10 || Math.abs(p.x - r.x) < 1e-10 || Math.abs(q.x - r.x) < 1e-10) {
      throw new Error('3点のx座標はすべて異なる値にしてください。');
    }
    return { P: p, Q: q, R: r };
  }

  function coefficientsFromPoints(points) {
    const x1 = points.P.x;
    const y1 = points.P.y;
    const x2 = points.Q.x;
    const y2 = points.Q.y;
  ...
  function getGraphDataSafe() {
    try {
      const points = parsePointInputs();
      const coefficients = coefficientsFromPoints(points);
      ...
      return { points: points, coefficients: coefficientsFromPoints(points) };
    }
  }
  ...
  function visibleParabolaPoints(a, b, c) {
    ...
    if (!points.length) {
      const x = Math.abs(a) > 1e-10 ? -b / (2 * a) : 0;
  ...
  function drawLineGraph(p1, p2) {
    ...
    const graph = getGraphDataSafe();
    const label = labelFromValue(state.sideInputs.f, formula(graph.coefficients.a, graph.coefficients.b, graph.coefficients.c));
    if (label) {
  ...
  function drawCurveGraph(a, b, c) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(graph.coefficients.a, graph.coefficients.b, graph.coefficients.c));
    if (label) {
      const anchor = points[Math.min(points.length - 1, Math.max(0, Math.floor(points.length * 0.88)))];
      const base = {
        x: Math.min(plot.right - 120, Math.max(plot.left + 16, anchor.x + 30)),
        y: Math.min(plot.bottom - 22, Math.max(plot.top + 34, anchor.y - 16))
      };
      drawText(label, base.x, base.y, 'muted', openLineSheet, { kind: 'side', id: 'f' });
    }
  }
  ...
  function render() {
    ...
    let graph;
    try {
      const inputPoints = parsePointInputs();
      graph = { points: inputPoints, coefficients: coefficientsFromPoints(inputPoints) };
      setStatus('3点 P, Q, R を通る二次関数を描画します。' + formula(graph.coefficients.a, graph.coefficients.b, graph.coefficients.c), false);
    } catch (error) {
      setStatus(error.message, true);
      graph = getGraphDataSafe();
    }
    drawCurveGraph(graph.coefficients.a, graph.coefficients.b, graph.coefficients.c);
    drawPoint('P', point(graph.points.P.x, graph.points.P.y));
    drawPoint('Q', point(graph.points.Q.x, graph.points.Q.y));
    drawPoint('R', point(graph.points.R.x, graph.points.R.y));
  }
  ...
  function openAngleSheet() {
    renderCommonSheet({
      ...
      angleValue: Math.abs(Math.atan(getGraphDataSafe().coefficients.a) * 180 / Math.PI),
      hint: '非表示、数値、比の値、自由入力を選べます。\n' + RATIO_LABEL_HINT
    });
  }
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-through-three-points-transparent.png' : 'function-through-three-points.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-through-three-points.pdf');
    });
  }
  ...
  pointQX.addEventListener('input', render);
  pointQY.addEventListener('input', render);
  pointRX.addEventListener('input', render);
  pointRY.addEventListener('input', render);
