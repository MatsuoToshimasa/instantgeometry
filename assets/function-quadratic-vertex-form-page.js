(function () {
  ...
  const vertexP = document.getElementById('vertexP');
  const vertexQ = document.getElementById('vertexQ');
  ...
  if (!stage || !coefA || !vertexP || !vertexQ) return;
  ...
  function parseVertexP() {
    const text = String(vertexP.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('頂点のx座標 p は数値で入力してください。');
    }
    return Number(text);
  }

  function parseVertexQ() {
    const text = String(vertexQ.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('頂点のy座標 q は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function formula(a, p, q) {
    if (Math.abs(a) < 1e-10) return 'y = 0';
    const coefficient = Math.abs(a - 1) < 1e-10 ? '' : (Math.abs(a + 1) < 1e-10 ? '-' : formatNumber(a));
    const inner = Math.abs(p) < 1e-10 ? 'x' : 'x ' + (p > 0 ? '- ' : '+ ') + formatNumber(Math.abs(p));
    const vertexTerm = coefficient + '(' + inner + ')²';
    if (Math.abs(q) < 1e-10) return 'y = ' + vertexTerm;
    return 'y = ' + vertexTerm + (q > 0 ? ' + ' : ' - ') + formatNumber(Math.abs(q));
  }
  ...
  function visibleParabolaPoints(a, p, q) {
    const points = [];
    const steps = 360;
    ...
    if (!points.length) points.push(point(p, q));
    return points;
  }
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseCoefficientSafe()));
    if (label) {
  ...
  function drawCurveGraph(a, p, q) {
    const points = visibleParabolaPoints(a, p, q);
    ...
    stage.appendChild(hit);
    const label = labelFromValue(state.sideInputs.f, formula(parseCoefficientSafe(), parseVertexPSafe(), parseVertexQSafe()));
    if (label) {
  ...
  function render() {
    ...
    try {
      ...
      p = parseVertexP();
      q = parseVertexQ();
      setStatus('係数 a と頂点 (p, q) を入力すると、放物線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-vertex-form-transparent.png' : 'function-vertex-form.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-vertex-form.pdf');
    });
  }
  ...
  vertexP.addEventListener('input', render);
  vertexQ.addEventListener('input', render);
