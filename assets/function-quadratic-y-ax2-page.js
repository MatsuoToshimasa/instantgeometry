(function () {
  ...
  function formula(a) {
    ...
    if (Math.abs(a - 1) < 1e-10) return 'y = x²';
    if (Math.abs(a + 1) < 1e-10) return 'y = -x²';
  ...
  function render() {
    ...
    try {
      a = parseCoefficient();
      setStatus('係数 a を入力すると、原点を頂点にもつ放物線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-y-ax2-transparent.png' : 'function-y-ax2.png';
