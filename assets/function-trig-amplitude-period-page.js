(function () {
  ...
  function parseAxisA() {
    ...
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('振幅 a は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function visibleSinePoints(a, b) {
    const points = [];
    const steps = 720;
    for (let i = 0; i <= steps; i += 1) {
      const x = plot.xMin + (plot.xMax - plot.xMin) * (i / steps);
      const y = a * Math.sin(b * x);
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      b = parseAxisB();
      setStatus('振幅 a と周期係数 b を入力すると、正弦曲線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-trig-amplitude-period-transparent.png' : 'function-trig-amplitude-period.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-trig-amplitude-period.pdf');
    });
  }
