(function () {
  ...
  function formula(a, b, p, q) {
    ...
    return 'y = ' + coefficient + 'cos ' + innerText(b, p) + signedText(q);
  }
  ...
  function visibleCosinePoints(a, b, p, q) {
    const points = [];
    const steps = 720;
    for (let i = 0; i <= steps; i += 1) {
      const x = plot.xMin + (plot.xMax - plot.xMin) * (i / steps);
      const y = a * Math.cos(b * (x - p)) + q;
  ...
  function drawCurveGraph(a, b, p, q) {
    const points = visibleCosinePoints(a, b, p, q);
  ...
  function render() {
    ...
    try {
      ...
      p = parseAxisP();
      q = parseAxisQ();
      setStatus('パラメータ a, b, p, q を入力すると、余弦曲線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-trig-cos-general-transparent.png' : 'function-trig-cos-general.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-trig-cos-general.pdf');
    });
  }
