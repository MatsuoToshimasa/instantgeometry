(function () {
  ...
  function visibleCosinePoints(a, b) {
    const points = [];
    const steps = 720;
    for (let i = 0; i <= steps; i += 1) {
      const x = plot.xMin + (plot.xMax - plot.xMin) * (i / steps);
      const y = a * Math.cos(b * x);
  ...
  function drawCurveGraph(a, b) {
    const points = visibleCosinePoints(a, b);
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      b = parseAxisB();
      setStatus('振幅 a と周期係数 b を入力すると、余弦曲線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-trig-cos-transparent.png' : 'function-trig-cos.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-trig-cos.pdf');
    });
  }
