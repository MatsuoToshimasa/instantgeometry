(function () {
  ...
  function formula(m, n) {
    return 'y = sin(' + angleText(m) + ')sin(' + angleText(n) + ')';
  }
  ...
  function visibleSinePoints(m, n) {
    ...
    for (let i = 0; i <= steps; i += 1) {
      ...
      const y = Math.sin(m * x) * Math.sin(n * x);
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      b = parseAxisB();
      setStatus('パラメータ m と n を入力すると、y = sin(mx)sin(nx) を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-trig-orthogonality-2-transparent.png' : 'function-trig-orthogonality-2.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-trig-orthogonality-2.pdf');
    });
  }
