(function () {
  ...
  function formula(m, n) {
    return 'y = cos(' + angleText(m) + ')cos(' + angleText(n) + ')';
  }
  ...
  function visibleCurvePoints(m, n) {
    const points = [];
    const steps = 720;
    for (let i = 0; i <= steps; i += 1) {
      const x = plot.xMin + (plot.xMax - plot.xMin) * (i / steps);
      const y = Math.cos(m * x) * Math.cos(n * x);
  ...
  function drawCurveGraph(a, b) {
    const points = visibleCurvePoints(a, b);
  ...
  function render() {
    ...
    try {
      a = parseAxisA();
      b = parseAxisB();
      setStatus('パラメータ m と n を入力すると、y = cos(mx)cos(nx) を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-trig-orthogonality-3-transparent.png' : 'function-trig-orthogonality-3.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-trig-orthogonality-3.pdf');
    });
  }
