(function () {
  ...
  if (!stage || !axisA || !axisB || !axisP || !axisQ) return;
  ...
  function formula(a, b, p, q) {
    ...
    return 'y = ' + coefficient + 'tan ' + innerText(b, p) + signedText(q);
  }
  ...
  function visibleTanSegments(a, b, p, q) {
    const segments = [];
    let points = [];
    const steps = 1600;
    for (let i = 0; i <= steps; i += 1) {
      ...
      const y = a * Math.tan(angle) + q;
      if (Math.abs(cos) < 0.018 || !Number.isFinite(y) || y < plot.yMin - 1e-9 || y > plot.yMax + 1e-9) {
  ...
  function visibleAsymptotes(b, p) {
    const lines = [];
    const start = Math.floor((b * (plot.xMin - p) - Math.PI / 2) / Math.PI) - 1;
    const end = Math.ceil((b * (plot.xMax - p) - Math.PI / 2) / Math.PI) + 1;
  ...
  function drawCurveGraph(a, b, p, q) {
    drawAsymptotes(b, p);
    const segments = visibleTanSegments(a, b, p, q);
  ...
  function render() {
    ...
    try {
      ...
      p = parseAxisP();
      q = parseAxisQ();
      setStatus('パラメータ a, b, p, q を入力すると、正接曲線と漸近線を描画します。', false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? 'function-trig-tan-general-transparent.png' : 'function-trig-tan-general.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save('function-trig-tan-general.pdf');
    });
  }
  ...
  axisP.addEventListener('input', render);
  axisQ.addEventListener('input', render);
