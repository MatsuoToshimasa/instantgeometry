(function () {
  ...
  const axisP = document.getElementById('axisP');
  const axisQ = document.getElementById('axisQ');
  ...
  const pageConfig = Object.assign({
    ...
    formulaName: 'tan',
    curveTitle: '関数グラフ',
    statusText: 'パラメータ a, b, p, q を入力すると、関数グラフを描画します。',
    saveBase: 'function-trig-transformed',
    params: ['a', 'b', 'p', 'q'],
    defaults: { a: 1, b: 1, p: 0, q: 0 }
  }, window.InstantGeometryTrigTransformedConfig || {});
  const activeParams = Array.isArray(pageConfig.params) ? pageConfig.params : ['a', 'b', 'p', 'q'];

  if (!stage) return;
  if (activeParams.indexOf('a') >= 0 && !axisA) return;
  if (activeParams.indexOf('b') >= 0 && !axisB) return;
  if (activeParams.indexOf('p') >= 0 && !axisP) return;
  if (activeParams.indexOf('q') >= 0 && !axisQ) return;
  ...
  const state = {
    ...
    areaColor: '#2a5bd7',
    tickLabelInterval: 1,
    xTickLabelMode: pageConfig.xTickLabelMode === 'pi' ? 'pi' : '',
    viewCenterX: 0,
    viewCenterY: 0,
    viewWidth: 20,
    viewHeight: 20,
  ...
  function parseAxisA() {
    if (!axisA) return Number(pageConfig.defaults.a || 1);
  ...
  function parseAxisB() {
    if (!axisB) return Number(pageConfig.defaults.b || 1);
    const text = String(axisB.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('周期係数 b は数値で入力してください。');
    }
    const value = Number(text);
    if (Math.abs(value) < 1e-10) throw new Error('周期係数 b は0以外の数値で入力してください。');
    return value;
  }

  function parseAxisP() {
    if (!axisP) return Number(pageConfig.defaults.p || 0);
    const text = String(axisP.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('移動 p は数値で入力してください。');
    }
    return Number(text);
  }

  function parseAxisQ() {
    if (!axisQ) return Number(pageConfig.defaults.q || 0);
    const text = String(axisQ.value || '').trim();
    if (!/^-?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)$/.test(text)) {
      throw new Error('移動 q は数値で入力してください。');
    }
    return Number(text);
  }
  ...
  function innerText(b, p) {
    const bx = Math.abs(b - 1) < 1e-10 ? 'x' : (Math.abs(b + 1) < 1e-10 ? '-x' : formatNumber(b) + 'x');
    if (Math.abs(p) < 1e-10) return bx;
    const shift = p > 0 ? ' - ' + formatNumber(p) : ' + ' + formatNumber(Math.abs(p));
    return Math.abs(b - 1) < 1e-10 ? 'x' + shift : formatNumber(b) + '(x' + shift + ')';
  }

  function formula(a, b, p, q) {
    if (Math.abs(a) < 1e-10) return 'y = 0';
    const coefficient = Math.abs(a - 1) < 1e-10 ? '' : (Math.abs(a + 1) < 1e-10 ? '-' : formatNumber(a));
    return 'y = ' + coefficient + pageConfig.formulaName + ' ' + innerText(b, p) + signedText(q);
  }
  ...
  function baseValue(kind, u) {
    if (kind === 'sin') return Math.sin(u);
    if (kind === 'cos') return Math.cos(u);
    if (kind === 'tan') return Math.tan(u);
    if (kind === 'arcsin') return Math.abs(u) <= 1 ? Math.asin(u) : NaN;
    if (kind === 'arccos') return Math.abs(u) <= 1 ? Math.acos(u) : NaN;
    if (kind === 'arctan') return Math.atan(u);
    if (kind === 'sinh') return (Math.exp(u) - Math.exp(-u)) / 2;
    if (kind === 'cosh') return (Math.exp(u) + Math.exp(-u)) / 2;
    if (kind === 'tanh') return Math.tanh ? Math.tanh(u) : (Math.exp(2 * u) - 1) / (Math.exp(2 * u) + 1);
    if (kind === 'cot') return 1 / Math.tan(u);
    if (kind === 'sec') return 1 / Math.cos(u);
    if (kind === 'cosec') return 1 / Math.sin(u);
    return Math.sin(u);
  }

  function isNearDiscontinuity(kind, u) {
    if (kind === 'tan' || kind === 'sec') return Math.abs(Math.cos(u)) < 0.018;
    if (kind === 'cot' || kind === 'cosec') return Math.abs(Math.sin(u)) < 0.018;
    return false;
  }

  function visibleCurveSegments(a, b, p, q) {
    const segments = [];
    let points = [];
    const steps = 1600;
    for (let i = 0; i <= steps; i += 1) {
      ...
      const u = b * (x - p);
      const y = a * baseValue(pageConfig.kind, u) + q;
      if (isNearDiscontinuity(pageConfig.kind, u) || !Number.isFinite(y) || y < plot.yMin - 1e-9 || y > plot.yMax + 1e-9) {
  ...
  function visibleAsymptotes(b, p) {
    if (!/^(tan|sec|cot|cosec)$/.test(pageConfig.kind)) return [];
    const lines = [];
    const offset = pageConfig.kind === 'tan' || pageConfig.kind === 'sec' ? Math.PI / 2 : 0;
    const start = Math.floor((b * (plot.xMin - p) - offset) / Math.PI) - 1;
    const end = Math.ceil((b * (plot.xMax - p) - offset) / Math.PI) + 1;
    for (let k = start; k <= end; k += 1) {
      const x = p + (offset + k * Math.PI) / b;
      if (x > plot.xMin + 1e-9 && x < plot.xMax - 1e-9) lines.push(x);
    }
    return lines;
  }
  ...
  function drawLineGraph(p1, p2) {
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseAxisASafe(), parseAxisBSafe(), parseAxisPSafe(), parseAxisQSafe()));
    if (label) {
  ...
  function drawAsymptotes(b, p) {
    visibleAsymptotes(b, p).forEach(function (x) {
      const line = svg('line', {
        x1: sx(x),
        y1: sy(plot.yMin),
        x2: sx(x),
  ...
  function drawCurveGraph(a, b, p, q) {
    drawAsymptotes(b, p);
    const segments = visibleCurveSegments(a, b, p, q);
    ...
    const label = labelFromValue(state.sideInputs.f, formula(parseAxisASafe(), parseAxisBSafe(), parseAxisPSafe(), parseAxisQSafe()));
    if (label) {
      const points = segments.reduce(function (best, segment) {
  ...
  function render() {
    ...
    try {
      ...
      p = parseAxisP();
      q = parseAxisQ();
      setStatus(pageConfig.statusText, false);
    } catch (error) {
  ...
  function saveImage(format) {
    ...
    return captureCanvas(transparent ? null : '#ffffff').then(function (canvas) {
      ...
      link.download = transparent ? pageConfig.saveBase + '-transparent.png' : pageConfig.saveBase + '.png';
  ...
  function savePdf() {
    ...
    return captureCanvas('#ffffff').then(function (canvas) {
      ...
      pdf.save(pageConfig.saveBase + '.pdf');
    });
  }
  ...
  if (axisA) axisA.addEventListener('input', render);
  if (axisB) axisB.addEventListener('input', render);
  if (axisP) axisP.addEventListener('input', render);
  if (axisQ) axisQ.addEventListener('input', render);
